...
- Production expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back. The private draft/RPC contract and child lifecycle columns exist, and contract migration `supabase/migrations/20260719015650_project_aggregate_write_lockdown.sql` is not applied. Initial apply left zero private drafts and all existing Project counts/statuses unchanged; the later approved Preview workflow now leaves one previously published tagged Project aggregate Archived.
- The 2026-07-19 security advisor readback reports the existing leaked-password-protection Auth warning plus two expected generic warnings for the deliberately browser-callable, slug-only archived-project tombstone function. A never-published archived draft could disclose its slug. The current private draft is the previously published QA Project, not that private-only case; the general residual must still be removed or explicitly accepted before production runtime promotion.
...
- `NOW-ADMIN-UX-RESHAPE-001`: the approved page-shaped `/admin/projects` Phase 1 source candidate is implemented; its host-side local suite, deployment-bound no-write smoke, and approved authenticated Preview happy path pass. Expand migration A is also applied/read back. Jay approved a one-time Docker-gate exception for the original branch push. Keep the task open while conflict/failure recovery, production promotion, contract B/readback, tombstone minimum-disclosure decision, and Jay-owned fool test are outstanding.
...
Required product action: expand migration A and the approved authenticated Preview happy path are complete. Keep all Project editing frozen while the remaining conflict/failure-recovery evidence is closed and Jay chooses whether to narrow or explicitly accept the private-only archived-slug disclosure. Then promote the matching aggregate UI/endpoint to production, obtain fresh approval before applying contract migration `supabase/migrations/20260719015650_project_aggregate_write_lockdown.sql`, and read back privileges/policies before lifting the freeze. Once B is applied, do not Cloudflare-only roll back to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Jay's fool test remains the acceptance and cannot be self-certified by an agent.
