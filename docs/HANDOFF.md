...
- The production custom origin is not yet healthy for that deployment: exact cached URLs for the global CSS, `publicMediaUrl`, and `publicContentOverlay` returned the SPA HTML shell with HTTP 200, causing a public dynamic-import failure. The old smoke accepted status-only false positives. MIME/body validation plus new hashes are prepared and must be deployed and re-read before production is called green.
...
- The immutable PR `#5` deployment proves nine authenticated routes plus sign-out and static public fallback under a blocked Supabase chunk. The production custom domain is temporarily blocked by the cached-HTML asset incident above; editor writes, Storage role boundaries, valid invite/recovery callbacks, and the twelve UI golden workflows are also not proven. `docs/agent/admin-handoff-evidence.json` intentionally remains `revalidation_required`.
...
- `NOW-ADMIN-RELIABILITY-UX-001`: P0 admin incident. The repaired build and Harness verifier are deployed and a separate QA Editor is active; first deploy and production-read back the MIME-aware cached-asset repair, then correct Auth callback configuration, complete the Storage prerequisite, and prove the editor golden workflows. Do not mark done from deployment, account existence, or source checks alone.
