...
- A current local follow-up fixes the public navbar/menu transparency regression at the Tailwind opacity-token source and adds built-CSS regression checks. It also removes Project proof-review controls and Dashboard claim-review queues: Save normalizes the legacy columns automatically, while required content/media, auth, RLS, promotion, conflict, and audit checks remain. This follow-up is not yet pushed or deployed; it still needs the clean container gate, immutable Preview smoke, and Jay's production-promotion approval.
...
Required delivery action: finish the local gate and immutable Preview for the opacity/admin-simplification follow-up, then obtain Jay's approval before promoting it to production. After promotion, Jay runs the Projects fool test. Do not Cloudflare-only roll back to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Jay's fool test remains the acceptance and cannot be self-certified by an agent.
