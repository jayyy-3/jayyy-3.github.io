...
- `NOW-PUBLIC-CATALOG-INTEGRITY-001`: draft PR `#36` and immutable Preview `https://085a75df.urblo-site.pages.dev` contain the full repair plus Jay's Steel Blue → BlueOcean canonical merge. Clean gate, deployed smoke, desktop/390px rendered checks, 12-card/one-BlueOcean listing readback, seven distinct finish images, and retired-route 301 pass. Obtain Jay acceptance and real iOS/Android WeChat result, then request separate approval for exact CMS Draft corrections and the duplicate-row merge/delete before production promotion.
