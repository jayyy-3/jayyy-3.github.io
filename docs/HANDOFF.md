...
- PR `#11` removes Project proof-review controls and Dashboard claim-review queues; Save normalizes the legacy columns automatically while required content/media, auth, RLS, promotion, conflict, and audit checks remain. Jay rejected its first public-header Preview because Projects did not use the Stone Library backing treatment. Commit `9d93624` corrected both Projects routes to the same 102px black `DefaultLayout` support band and removed their duplicate top offsets, including the stale detail loading offset. After Jay's approval, PR `#11` merged as `59cded9b` and deployed as immutable production `76de2abf-a27b-4ecb-9e1f-fe229af4c8ed`; the immutable URL, apex, and `www` pass deployment-bound smoke, production owner login passes all nine Admin routes, and desktop/mobile Projects rendering has no horizontal overflow.
...
- `NOW-ADMIN-UX-RESHAPE-001`: the approved page-shaped `/admin/projects` Phase 1 runtime is deployed. PR `#11` now aligns public Projects chrome with Stone Library and simplifies Projects to Edit/Save/Preview/Publish/Hide without claim approval UI in production. Keep the task open through Jay's fool test.
...
Required delivery action: Jay runs the production Projects fool test on merge `59cded9b` / immutable deployment `76de2abf-a27b-4ecb-9e1f-fe229af4c8ed`. Do not Cloudflare-only roll back to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Jay's fool test remains the acceptance and cannot be self-certified by an agent.
