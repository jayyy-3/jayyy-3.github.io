...
- `NOW-ADMIN-UX-RESHAPE-001`: the approved page-shaped `/admin/projects` Phase 1 source candidate is implemented and its host-side local suite passes. Keep the task open while the clean-container gate, branch preview, both aggregate rollout migrations, authenticated workflow, production/contract evidence, and Jay-owned fool test are outstanding.
