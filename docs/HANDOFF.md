...
- `/admin/settings` source is implemented as the first CRUD screen for the default `site_settings` row. Supabase migration `admin_settings_role_hardening` is applied so settings writes require owner/admin.
...
- `/admin/media` source is implemented as a protected media library screen for upload-backed draft records, external records, metadata editing, and publish/archive validation. Live upload/save verification still requires browser-safe Supabase key configuration plus an active admin/editor profile.
...
- Last full runtime gate on 2026-05-28: `npm run build`, `npm run lint`, `npx tsc -b`, and `npm run agent:smoke` pass during the admin media checkpoint. Build keeps the previous healthy JavaScript chunk profile; Browserslist staleness notice remains.
...
- Supabase foundation schema/RLS, baseline seeds, admin settings role hardening, and media Storage policies are applied and verified. Form API source is implemented, but live row creation still needs server-side environment variables. Admin auth/settings/media source is implemented, but live auth/profile/save/upload verification still needs browser-safe Supabase key configuration and the first admin email/profile. Broader content CRUD, lead inbox, and transactional email verification are not implemented yet.
...
- The admin shell exists, and Settings/Media are the first source CRUD surfaces, but the admin CMS is not customer-operational yet. `NOW-ADMIN-CMS-001` is an umbrella objective; the auth shell is config-gated and uses real Supabase Auth/RLS when browser-safe config exists. First-admin verification must wait for Jay to confirm the admin email and for browser-safe key configuration.
...
Continue `NOW-FORMS-BACKEND-001` by configuring a server-side `SUPABASE_SERVICE_ROLE_KEY` for local/Cloudflare Pages Function verification, then run live API tests proving valid enquiries and sample requests create Supabase rows and invalid submissions create no rows. In parallel, continue `NOW-ADMIN-AUTH-RLS-001` only after Jay confirms the first admin email and browser-safe Supabase key configuration (`VITE_SUPABASE_ANON_KEY` or `VITE_SUPABASE_PUBLISHABLE_KEY`) is available; then verify unauthenticated redirect, unprofiled unauthorized, active admin dashboard access, `/admin/settings` save, `/admin/media` upload/save, and `/admin/stone-library` save flows. If continuing without credentials, move to the next source-only CRUD checkpoint in the requested order: Projects with material maps/hotspots behind the existing auth/config gate. Optional `TURNSTILE_SECRET_KEY`, `RESEND_API_KEY`, `LEAD_NOTIFICATION_FROM`, `ENQUIRY_NOTIFICATION_TO`, and `SAMPLE_REQUEST_NOTIFICATION_TO` can be added after the database write path is verified. Keep article claim cleanup and broad legacy project migration paused until the user resumes them. Start Cloudflare production deployment/DNS cutover only after Supabase-backed form behavior is verified or the user explicitly accepts a static-only launch.
