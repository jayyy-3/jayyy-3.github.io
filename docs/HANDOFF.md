...
- Jay approved the full Projects count/data normalization on 2026-08-03. Current source changes the default admin list from raw `All` rows to non-Archived `Projects`, uses Drafts/Live filters, and exposes Archive only while archived rows exist; the verifier rejects the old All/Saved/Hidden filter language. Approved production actions remain pending Preview: publish the four canonical Draft Projects and permanently delete exactly four confirmed archived QA Projects, followed by exact 5-Live/0-Archived and public-five readback.
...
Required delivery action: complete the approved Projects count/data normalization through clean-container gate, immutable Preview, four canonical publishes, exact four-QA deletion, production UI promotion, and readback. Then begin Phase 2 with Articles, replicating the Jay-accepted Projects pattern without weakening content/media, auth, RLS, audit, or public-read boundaries. Keep `docs/ADMIN_UX_RESHAPE_PLAN.md` until every later phase is absorbed into canonical docs and its sunset clause is satisfied.
