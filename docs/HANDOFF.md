...
- `NOW-ADMIN-UX-RESHAPE-001`: the approved page-shaped `/admin/projects` Phase 1 source candidate is implemented and its host-side local suite passes. Jay approved a one-time Docker-gate exception for the current branch push; keep the task open while the branch preview, both aggregate rollout migrations, authenticated workflow, production/contract evidence, and Jay-owned fool test are outstanding.
...
Required product action: use Jay's one-time Docker-gate exception to push the current Phase 1 branch and complete its no-write route/asset/API smoke. Then stop for Jay's separate approval before applying/read-back of expand migration `supabase/migrations/20260714052955_project_aggregate_drafts.sql`; its lifecycle backfill is a production write. Any tagged authenticated Project/Storage workflow needs a second action-specific approval. Before those preview writes begin, freeze all Project editing and keep that freeze through authenticated preview evidence, aggregate UI/endpoint production promotion, contract migration `supabase/migrations/20260714052956_project_aggregate_write_lockdown.sql`, and privilege/policy readback. Contract B needs its own fresh Jay approval. Once contract is applied, do not Cloudflare-only roll back to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Jay's fool test remains the acceptance and cannot be self-certified by an agent.
