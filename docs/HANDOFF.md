...
- Production expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back. The private draft/RPC contract and child lifecycle columns exist. Minimum-disclosure migration `supabase/migrations/20260719015650_restrict_archived_project_tombstones.sql` and contract migration `supabase/migrations/20260719015651_project_aggregate_write_lockdown.sql` are source-only and not applied. Initial A apply left zero private drafts and all existing Project counts/statuses unchanged; the later approved Preview workflow now leaves one previously published tagged Project aggregate Archived.
- The 2026-07-19 security advisor readback reports the existing leaked-password-protection Auth warning plus two expected generic warnings for the browser-callable archived-project tombstone function. Read-only production proof found that the current A implementation returns four QA slugs, including one never-published slug, so the disclosure is concrete rather than hypothetical. Migration C now limits the result to archived canonical rows intersecting the five already-public bundled fallback slugs, and `ProjectService` independently rejects non-static results; neither repair is deployed/applied yet.
...
- Phase 1's aggregate runtime has not reached production. Expand migration A is applied/read back; C and B remain source-only, separately approval-gated, and unapplied. The host-side suite passes, including a read-only 1116px/mobile check and deterministic stale-Save 409, conflict lock/reload-only recovery, and full mocked failed-Publish Storage compensation. Real two-session/Postgres/Storage failure proof remains a separately approval-gated live action. The post-commit container gate could not start because Docker is absent; on 2026-07-16 Jay approved a one-time exception for that original branch push only. Commit `9441811` was rebuilt with Preview-only Supabase secrets as deployment `1a3e0d4b-d74a-4979-be64-921e5a510ccc`; its no-write smoke and owner login passed. The approved marker `admin-projects-ui-mrroa6p0` then passed exactly one Save/refresh, unsaved shared preview, private-first upload, 55/55 hotspot, Publish/public readback, and Hide/public-not-found. Because it was not a bundled fallback slug, it did not prove the live tombstone path. Final Project/draft/children are Archived, the private upload source is removed, and the promoted Media/public object remain under the archive-first retention policy. This is authenticated Preview implementation evidence, not production handoff or Jay's fool test; C, aggregate runtime promotion, B privilege/policy readback, and user acceptance remain open.
...
- `NOW-ADMIN-UX-RESHAPE-001`: the approved page-shaped `/admin/projects` Phase 1 source candidate is implemented; its host-side local suite, deterministic conflict/compensation checks, deployment-bound no-write smoke, and approved authenticated Preview happy path pass. Expand migration A is applied/read back; minimum-disclosure C and contract B are not. Jay approved a one-time Docker-gate exception for the original branch push. Keep the task open through a new gated Preview, C approval/readback, production promotion, B approval/readback, any separately approved live negative proof, and Jay's fool test.
...
Required product action: source-side conflict recovery, failed-Publish compensation, and the tombstone minimum-disclosure solution are now implemented. Keep all Project editing frozen. First pass a fresh delivery gate and deploy/verify a new immutable Preview. Then obtain separate approvals for applying/read back migration C, promoting the matching aggregate UI/endpoint to production, and finally applying/read back contract migration `supabase/migrations/20260719015651_project_aggregate_write_lockdown.sql`. When C is approved, use a single-migration path that applies exactly C; a bulk pending-migration command would also apply still-unapproved B. Real two-session/Storage negative writes remain optional but require their own fresh approval. Do not lift the freeze until B privileges/policies are verified. Once B is applied, do not Cloudflare-only roll back to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Jay's fool test remains the acceptance and cannot be self-certified by an agent.
