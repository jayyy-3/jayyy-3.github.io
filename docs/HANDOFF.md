...
Local candidate: Project image delivery now preserves CMS originals and serves public card/list/detail/hero/hotspot images through quality-first responsive Supabase WebP profiles. Desktop and 390px local Browser checks pass with no relevant console errors or horizontal overflow. The Admin upload copy says the original is retained and high-quality website versions are prepared automatically. Commit `c383983` passes build, lint, typecheck, smoke, Admin predeploy/config, Harness, and the clean Node 20 container gate. No production content, Storage object, Supabase configuration, or deployment has changed; publish and verify a branch Preview before any promotion.
...
Publish the gated Project image-delivery candidate as a branch Preview and run deployment-bound smoke plus desktop/390px visual checks for Jay's acceptance. Do not rewrite or delete any existing Project original; production should continue using the current runtime until that Preview passes.
