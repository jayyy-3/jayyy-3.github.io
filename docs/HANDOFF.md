...
- Jay approved the bounded Projects count/data normalization on 2026-08-03. PR `#14` Preview passed route/asset/Function smoke, authenticated desktop list/filter checks, and 390px no-overflow readback. Australian Catholic University, Xavier College, and Artisan Park | YarraBend were published through the protected Admin UI. Moon Gate remains Draft because its two referenced Stone Library groups are not Published and its imported hotspots are not yet material-bound; the server/client publish guard was preserved. A guarded transaction then deleted only the four exact Archived QA Projects. Production readback is now five canonical Projects, four Live, one Draft, zero Archived/QA rows, with retained media/audit history; the public Preview still shows all five through the explicit Published-overlay/static-fallback contract. The UI source defaults to non-Archived Projects, uses Drafts/Live filters, and exposes Archive only while Archived rows exist.
