...
Image QR runtime is implemented on draft PR `#29` / branch `codex/admin-image-qr-library`: the new `/admin/image-qr` workspace bulk-optimizes images, creates one stable direct-image QR per file, supports PNG/SVG downloads, rename, replace, and Hide/Restore, and keeps privileged work behind protected Functions. Separately approved production migration `supabase/migrations/20260818074046_image_qr_resources.sql` is applied/read back: the empty server-only table has RLS enabled, no browser-role privileges or policies, service-role CRUD, the expected trigger, and four expected indexes. Immutable Preview `https://35044c9d.urblo.pages.dev` passed complete no-write route/asset/MIME/redirect/Function/secret-boundary smoke, including `/admin/image-qr` and its protected Function. No Image QR database row, Storage object, or audit event has been created. Tagged live upload/replace checks and runtime promotion remain separate approval boundaries.
...
Commit `842c11e` passes the clean Node 20 container gate, production migration `20260818074046_image_qr_resources.sql` is applied/read back, and draft PR `#29` Preview `https://35044c9d.urblo.pages.dev` passes no-write smoke. A tagged upload/replace/Hide/Restore workflow now requires its own explicit approval before any Image QR row or Storage object is created; then give Jay the page for the fool test. Do not promote the runtime to production without separate approval.
