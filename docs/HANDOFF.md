...
- `NOW-PROJECT-STONE-POINTS-001`: implementation is in progress on `codex/project-hotspots-stone-library-source`. The Admin material flow is Stone → Variant → supported Finish with a read-only Stone Library image; Project-owned material/hotspot image and title overrides are retired in runtime. Production read-only inspection confirmed Moon Gate's two exact Stone Library material/finish/image chains exist as Draft, so editing now admits non-Archived records and labels Draft/Live state while publishing still requires the full Stone/Variant/Finish/image/media chain to be Published. Commit `f7b8d70` passed the clean Docker gate and immutable Preview `https://0e8e4fa0.urblo.pages.dev` passed no-write smoke. The compatibility migration backfills a unique Draft or Live variant without publishing content. It is not applied to production; applying it requires Jay's separate approval.
