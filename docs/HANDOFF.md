...
Image QR is implemented only on branch `codex/admin-image-qr-library`: the new `/admin/image-qr` workspace bulk-optimizes images, creates one stable direct-image QR per file, supports PNG/SVG downloads, rename, replace, and Hide/Restore, and keeps privileged work behind protected Functions. Production is unchanged. Migration `supabase/migrations/20260818070944_image_qr_resources.sql` is prepared but unapplied; no Image QR database row, Storage object, or audit event has been created. Applying it, running live upload/replace checks, and promoting the runtime each require their own approval boundary.
...
Commit `842c11e` passes the clean Node 20 container gate for the Image QR source candidate. The next action is Jay's separate approval to apply `20260818070944_image_qr_resources.sql`. After migration readback, deploy the branch Preview, run no-write smoke plus an explicitly approved tagged upload/replace/Hide/Restore workflow, and give Jay the page for the fool test. Do not promote to production or apply the migration from this source checkpoint without those separate approvals.
