...
- Production expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back. The private draft/RPC contract and child lifecycle columns exist, the draft table has 0 rows, existing Project/child counts and the Project status split are unchanged, child lifecycle values match their parents, and contract migration `supabase/migrations/20260719015650_project_aggregate_write_lockdown.sql` is not applied.
- The 2026-07-19 security advisor readback reports the existing leaked-password-protection Auth warning plus two expected generic warnings for the deliberately browser-callable, slug-only archived-project tombstone function. A never-published archived draft could disclose its slug; there are currently no draft rows, and this narrow residual must be removed or explicitly accepted before production runtime promotion.
...
- Phase 1 has not reached production. Expand migration A is applied/read back; contract migration B remains source-only, separately approval-gated, and unapplied. The full host-side local suite passes, including a read-only 1116px/mobile implementation check. The post-commit container gate could not start because Docker is absent; on 2026-07-16 Jay approved a one-time exception for this branch push only. Latest deployed preview commit `9441811` serves at `https://d29d45cf.urblo.pages.dev` and passes the full no-write preview smoke, although GitHub's Cloudflare callback still displayed `in_progress` at final readback. Authenticated save/publish/public-readback/hide evidence, aggregate runtime promotion, privilege/policy readback after B, and Jay's unassisted fool test remain pending. No agent may turn migration/source checks or preview route smoke into handoff certification.
...
- `NOW-ADMIN-UX-RESHAPE-001`: the approved page-shaped `/admin/projects` Phase 1 source candidate is implemented; its host-side local suite and deployment-bound no-write branch-preview smoke pass, and expand migration A is applied/read back. Jay approved a one-time Docker-gate exception for the original branch push. Keep the task open while the separately approved authenticated workflow, production promotion, contract B/readback, tombstone minimum-disclosure decision, and Jay-owned fool test are outstanding.
...
Required product action: expand migration A is complete. Stop for Jay's separate approval before creating the cleanable tagged Project/Storage evidence. Before those writes begin, freeze all Project editing and keep that freeze through authenticated preview evidence, resolution or explicit acceptance of the private-only archived-slug disclosure, aggregate UI/endpoint production promotion, fresh approval/application of contract migration `supabase/migrations/20260719015650_project_aggregate_write_lockdown.sql`, and privilege/policy readback. Once B is applied, do not Cloudflare-only roll back to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Jay's fool test remains the acceptance and cannot be self-certified by an agent.
