...
- PR `#16` makes Stone Library origin an internal-only sourcing field: public cards, detail specs, search matching, and route metadata omit it without deleting the source/Supabase/Admin value. Local and Cloudflare Preview desktop/mobile UI checks plus Preview smoke pass; production confirmation follows merge.
