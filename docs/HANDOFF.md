...
- `NOW-PUBLIC-CATALOG-INTEGRITY-001`: Jay accepted Preview and approved promotion. PR `#36` squash-merged as `5731651`; Cloudflare production deployment `https://764dedbc.urblo-site.pages.dev`, apex, and `www` pass deployment-bound smoke. Production Browser QA confirms Homepage mobile playback, 12 canonical Stone cards with one BlueOcean/zero Steel Blue, seven distinct BlueOcean finish images, Tuscany Honed-only/Cross Cut without a top jump, and corrected Terra Line transitions. Real iOS/Android WeChat remains Jay's device check; exact CMS Draft corrections and the duplicate-row merge/delete still require separate production-write approval.
