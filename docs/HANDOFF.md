...
Required delivery action: begin Phase 2 with Articles, replicating the Jay-accepted Projects single-editor pattern without weakening its existing content/media, auth, RLS, audit, or public-read boundaries. Do not Cloudflare-only roll back Projects to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Keep `docs/ADMIN_UX_RESHAPE_PLAN.md` until every later phase is absorbed into canonical docs and its sunset clause is satisfied.
