...
Homepage Latest Projects now has a local release candidate that replaces its hand-maintained five-record cap with the complete Published Project collection in the existing CMS order. The query starts only as the section nears the viewport; the controlled five records remain the read-failure fallback. Local desktop/mobile Browser QA reads all 15 current public Projects, reaches item `15 / 15`, keeps the section at one viewport, resets the rail to the active first item after data replacement, and introduces no horizontal page overflow. Production is unchanged pending normal branch gate, Preview, and promotion.
