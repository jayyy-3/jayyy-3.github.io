...
Local candidate: Project image delivery now preserves CMS originals and serves public card/list/detail/hero/hotspot images through quality-first responsive Supabase WebP profiles. Desktop and 390px local Browser checks pass with no relevant console errors or horizontal overflow. The Admin upload copy says the original is retained and high-quality website versions are prepared automatically. No production content, Storage object, Supabase configuration, or deployment has changed; finish the gates and branch Preview before any promotion.
...
Finish the Project image-delivery candidate's complete source/container gates, commit only the scoped tracked changes, and publish a branch Preview for Jay's visual acceptance. Do not rewrite or delete any existing Project original; production should continue using the current runtime until that Preview passes.
