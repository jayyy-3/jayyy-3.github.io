...
- Supabase foundation schema/RLS, baseline seeds, and admin settings role hardening are applied and verified. Form API source is implemented, but live row creation still needs server-side environment variables. Admin auth/settings source is implemented, but live auth/profile/save verification still needs browser-safe Supabase key configuration and the first admin email/profile. Storage, broader admin CRUD, and transactional email verification are not implemented yet.
...
- The admin shell exists, but the admin CMS is not customer-operational yet. `NOW-ADMIN-CMS-001` is an umbrella objective; the auth shell is config-gated and uses real Supabase Auth/RLS when browser-safe config exists. First-admin verification must wait for Jay to confirm the admin email and for browser-safe key configuration.
