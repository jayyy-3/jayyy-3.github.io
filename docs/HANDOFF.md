...
- Draft PR `#11` removes Project proof-review controls and Dashboard claim-review queues; Save normalizes the legacy columns automatically while required content/media, auth, RLS, promotion, conflict, and audit checks remain. Jay rejected its first public-header Preview because Projects did not use the Stone Library backing treatment. Current source corrects both Projects routes to the same 102px black `DefaultLayout` support band and removes their duplicate top offsets, including the stale detail loading offset. Local desktop/mobile listing, menu, and detail checks pass; rerun the clean container and immutable Preview gates before requesting production promotion.
...
- `NOW-ADMIN-UX-RESHAPE-001`: the approved page-shaped `/admin/projects` Phase 1 runtime is deployed. The current local follow-up aligns public Projects chrome with Stone Library and simplifies Projects to Edit/Save/Preview/Publish/Hide without claim approval UI. Keep the task open through a fresh gate/Preview, Jay's promotion decision, and Jay's fool test.
...
Required delivery action: run a fresh clean-container and immutable Preview gate for Jay's Projects-header correction in draft PR `#11`, then obtain Jay's approval before production promotion. After promotion, Jay runs the Projects fool test. Do not Cloudflare-only roll back to the legacy direct-write UI; retain the aggregate runtime or use a separately approved forward-compatibility migration. Jay's fool test remains the acceptance and cannot be self-certified by an agent.
