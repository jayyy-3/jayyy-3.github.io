...
- `NOW-PROJECT-STONE-POINTS-001`: implementation is in progress on `codex/project-hotspots-stone-library-source`. The Admin material flow is Stone → Variant → supported Finish with a read-only Stone Library image; Project-owned material/hotspot image and title overrides are retired in runtime. A compatibility migration adds/backfills `project_materials.stone_variant_id` only for unambiguous old rows and validates the canonical relationship at publish. It is not applied to production; applying it requires Jay's separate approval after the gate.
