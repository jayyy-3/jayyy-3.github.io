...
Current branch candidate `codex/homepage-logo-marquee` adds the 12 supplied partner marks to the existing homepage marquee without changing its white-strip composition. Source uses two equal groups for a seamless loop, one accessible copy, and a reduced-motion fallback. Local desktop/390px rendering, build, lint, typecheck, and smoke pass; commit, Preview, and production remain pending Jay's visual approval.
