...
Commit `842c11e` passes the clean Node 20 container gate for the Image QR source candidate, and production migration `20260818074046_image_qr_resources.sql` is applied/read back. Next, deploy the branch Preview and run no-write smoke. A tagged upload/replace/Hide/Restore workflow still requires its own explicit approval before any Image QR row or Storage object is created; then give Jay the page for the fool test. Do not promote the runtime to production without separate approval.
