...
- The Phase 1 Projects source is not accepted by its own verifier. Expand migration `20260719015649_project_aggregate_drafts.sql` is applied/read back; next, under a separate tagged-write approval, freeze all Project editing and collect authenticated Save/refresh, preview parity, Publish/public readback including a saved material map/hotspot, Hide plus archived-slug suppression, failed/conflicting save recovery, and private-media promotion/compensation evidence. Keep the freeze through tombstone minimum-disclosure resolution or acceptance, aggregate runtime production promotion, a fresh approval/application of contract migration `20260719015650_project_aggregate_write_lockdown.sql`, and readback of all six table privileges, six sequence privileges, public policies, and the security advisor. B makes Cloudflare-only rollback to the old direct-write UI invalid. Jay alone passes the fool test; an implementing or reviewing agent must not self-certify it.
