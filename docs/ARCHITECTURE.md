...
- Supabase Auth invite/recovery mail is operationally separate from Contact/Sample Request SMTP2GO. Production requires Auth custom SMTP plus allowed Redirect URL entries for the exact account-setup URLs before either flow can be called verified.
...
- Supabase Auth shell and first-owner profile exist, but the prior browser proof predates the auth-listener and account-setup repair. Production invite/recovery also depends on unverified Supabase Auth custom SMTP and Redirect URL configuration. Auth/admin handoff remains `revalidation_required`.
...
- Public and admin route-level code splitting resolves the previous monolithic route bundle risk; continue monitoring shared chunks as editors are refactored.
