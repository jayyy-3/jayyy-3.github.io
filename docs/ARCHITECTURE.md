...
- Supabase Storage policies and `/admin/media` source are implemented, and approved live upload verification passed with `--include-storage`. Admin profile source management and owner-role RLS hardening are implemented; source CRUD screens now exist for Stone Library, Projects, Products, Articles, Leads, and Audit. Public Projects, Stone Library, Products, and Articles prefer published Supabase content with static fallback; imported production content remains draft until an editor publishes items in `/admin`.
