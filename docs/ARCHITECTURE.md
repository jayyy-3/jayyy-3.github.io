...
| `/admin/*` | `AdminApp` | Protected admin shell outside public site chrome. Config-gated until browser-safe Supabase key is set; uses Supabase Auth plus `admin_profiles` once configured. Current source CRUD screens: Settings, Media, Stone Library, Projects, Products. |
...
- Admin shell exists, with Settings, Media, Stone Library, Projects, and Products as the first source CRUD screens. Customer CRUD for Articles, leads, audit, and admin-user management is not implemented yet.
