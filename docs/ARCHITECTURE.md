...
- Admin CRUD live verification:
  ...
  - The command creates tagged QA rows, archives public-facing parents where possible, verifies those tagged public-content rows are not anonymously visible, and intentionally avoids physical deletes. Optional `--include-storage` uploads a tiny private `urblo-admin-media` object for Storage-policy verification.
