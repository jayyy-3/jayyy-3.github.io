...
- Project hotspot component: `src/components/projects/ProjectHotspotImage.tsx`
  ...
  - Hotspots are material-placement records keyed by `stoneGroupId`, `stoneVariantId`, and `finishKey`. Project owns the map image, point coordinates, application, and project note; Stone Library owns the material name, valid variant/finish relationship, finish preview image/alt, and detail link. Public cards load the current Published Stone Library detail with static fallback, and Project-specific legacy title/image fields no longer override that canonical material presentation.
