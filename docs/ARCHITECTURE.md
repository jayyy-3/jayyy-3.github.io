...
- Public access layer: `src/service/ProjectService.ts`
  ...
  - The adapter also reads `get_archived_project_slugs()`. Expand migration A is installed, but its production function currently over-discloses archived QA slugs. Source migration C restricts output to archived canonical rows intersecting the five bundled public fallback slugs, while `ProjectService` applies the same allowlist as defence in depth. C remains unapplied. Once C and the aggregate runtime are promoted, an allowlisted archived slug suppresses its matching bundled fallback. A read failure keeps the availability-first static fallback, and a Published CMS row wins over a stale tombstone.
...
- Public migration overlay:
  ...
  - Draft rows do not replace static migration fallback. Applied migration A added `get_archived_project_slugs()` so Archive can suppress a matching bundled Project, but production readback found four QA slugs including one never-published marker. Source migration `20260719015650_restrict_archived_project_tombstones.sql` removes private-draft reads and returns only the archived-canonical/five-public-fallback intersection; the public adapter rejects any unexpected slug independently. If the RPC is unavailable, source preserves healthy static pages. C remains separately approval-gated and unapplied.
  - Public Project material/map/hotspot consumption and shared draft/public rendering now exist in source. Deterministic local tests cover stale-Save 409, the conflict reload-only lock, and full failed-Publish public-copy compensation, but production parity still requires a fresh Preview, C readback, production runtime promotion, B privilege/policy readback, any separately approved live negative proof, and Jay's fool test.
- Admin IA/access:
  ...
  - The relational and private-draft contract from expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back. Minimum-disclosure C `supabase/migrations/20260719015650_restrict_archived_project_tombstones.sql` and write-lockdown B `supabase/migrations/20260719015651_project_aggregate_write_lockdown.sql` remain source-only and each requires fresh explicit production approval/readback.
  - The separately approved authenticated Preview happy path and deterministic local conflict/compensation behavior are complete. Remaining rollout order is fixed: keep all Project editing frozen; pass a fresh delivery gate and immutable Preview; separately approve/apply/read back C; promote the matching aggregate UI/endpoint to production under separate approval; separately approve/apply/read back B and verify table/sequence privileges plus public policies; then lift the freeze. Real two-session/Postgres/Storage negative proof is optional but needs its own approval. This continuous freeze prevents legacy child-table writes during the expand/contract overlap. Once B is applied, a Cloudflare-only rollback to the legacy direct-write UI is invalid. Runtime rollback must retain the aggregate UI/endpoint, or use a separately reviewed and approved forward-compatibility migration.
...
- Public Projects, Stone Library listing/detail, Products, and Articles use migration-safe Published overlays with static fallback in local source. Projects has an installed but over-broad archived-slug RPC; source migration C plus client allowlisting narrow it to the five public fallbacks before the aggregate runtime is promoted. Other content types still require an explicit CMS-only cutover/removal policy.
...
- Admin editors outside Projects remain large schema-shaped components. Projects is now the page-shaped aggregate prototype with one draft, one action bar, shared public/preview rendering, inline private media, and visual hotspots. Expand migration A is installed; local negative behavior is covered, but C, a fresh Preview, production promotion, contract B, and Jay's fool test have not accepted the pattern yet.
...
- Project public detail consumes approved material/map/hotspot relationships in source and shares its renderer with draft preview. The approved non-static QA marker proved Save/Publish/Hide/public-not-found but not the bundled-fallback tombstone path. Do not call Projects end-to-end complete until C is applied/read back, the matching runtime is in production, B is applied/read back, and Jay accepts the fool test; any live static-fallback or negative write proof needs fresh approval.
