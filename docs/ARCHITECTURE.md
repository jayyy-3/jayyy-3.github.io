...
- Admin IA/access:
  ...
  - Remaining rollout order is fixed: obtain separate tagged-write approval; freeze all Project editing; validate the authenticated preview workflow; narrow or accept the tombstone disclosure; promote the matching aggregate UI/endpoint to production; separately approve/apply/read back contract migration B and verify table/sequence privileges plus public policies; then lift the freeze. This continuous freeze prevents legacy child-table writes during the A/B overlap. Once B is applied, a Cloudflare-only rollback to the legacy direct-write UI is invalid. Runtime rollback must retain the aggregate UI/endpoint, or use a separately reviewed and approved forward-compatibility migration.
