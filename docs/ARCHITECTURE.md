...
- Supabase Storage policies and `/admin/media` source are implemented, but live upload verification still requires optional explicit `--include-storage` approval. Admin profile source management and owner-role RLS hardening are implemented; source CRUD screens now exist for Stone Library, Projects, Products, Articles, Leads, and Audit. Supabase cannot replace static/file-backed public content until approved content import and public read cutover are completed.
