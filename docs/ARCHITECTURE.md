...
- Current Supabase project: `Urblo` (`npkidywzwddbnfrnxlmo`, `ap-southeast-2`) has the foundation schema/RLS migrations, baseline seeds, admin settings/profile/helper hardening, admin profile email uniqueness, and media Storage role hardening applied. Production migration `supabase/migrations/20260714050750_media_public_bucket_role_hardening.sql` restricts public-bucket insert/update to owner/admin while retaining editor writes to the private draft bucket; migration history and both policy definitions were read back on 2026-07-14. The Projects aggregate rollout is split into source-only migrations: `supabase/migrations/20260714052955_project_aggregate_drafts.sql` is the expand step and `supabase/migrations/20260714052956_project_aggregate_write_lockdown.sql` is the later contract step. Neither is approved or applied. Jay must approve each production migration separately. The `/admin` auth/CRUD source and first owner profile exist, but the production editor handoff is `revalidation_required` after the 2026-07-13 not-working report. Prior route-shell and direct browser-key/API proofs remain infrastructure evidence; they do not prove the UI save/publish/public-readback workflow. Current closure is defined by `NOW-ADMIN-RELIABILITY-UX-001` and `docs/agent/admin-handoff-evidence.json`.
...
- Browser-side public form Turnstile uses `VITE_TURNSTILE_SITE_KEY`; without it, the Contact page keeps the submit flow usable and omits the widget. Server-side token verification still requires `TURNSTILE_SECRET_KEY` or `CF_TURNSTILE_SECRET_KEY`.
  ...
  - Current change-history write source: `src/lib/adminAudit.ts` inserts `admin_audit_events` after successful admin Settings, admin profile, Media, Stone Library, Products, Articles, and Leads mutations. Projects aggregate Save/Publish/Hide moves its audit row into the same pending database RPC transaction as the relational change, so the browser does not perform a second audit mutation. Outside Projects, change-history insert failures are appended to the success notice and do not roll back the already-saved primary change.
...
- Admin Projects aggregate coverage:
  ...
  - Includes executable mapping/blocker fixtures for preview/public parity and rejects direct browser child-table or Projects audit mutations. It is source-only: it does not apply either Projects migration, copy Storage objects, publish content, prove an authenticated deployed workflow, or pass Jay's fool test.
...
- Public access layer: `src/service/ProjectService.ts`
  ...
  - The adapter also reads the narrow `get_archived_project_slugs()` RPC. Once expand migration A and the aggregate runtime are deployed, an archived slug suppresses its matching bundled Project fallback; a tombstone read failure deliberately keeps the existing availability-first static fallback. A Published CMS row always wins over a stale tombstone.
...
- Projects:
  ...
  - The pending Phase 1 contract stores the editable page aggregate in `private.project_drafts` with an optimistic revision and canonical-project baseline timestamp. Draft JSON is not directly granted to browser roles; `/api/admin/projects` is the authenticated boundary.
...
- Public migration overlay:
  ...
  - Draft rows do not replace static migration fallback. The pending Projects migration adds `get_archived_project_slugs()` as a deliberately narrow tombstone list so Archive can suppress the matching bundled Project without exposing private drafts; if the tombstone RPC is unavailable, source falls back to the earlier availability-first behavior instead of hiding healthy static pages.
...
- Admin IA/access:
  ...
  - The relational and private-draft contract depends on expand migration `supabase/migrations/20260714052955_project_aggregate_drafts.sql`. Removing legacy browser writes and hardening the final public parent/child boundary depends on the later contract migration `supabase/migrations/20260714052956_project_aggregate_write_lockdown.sql`. Both are source-only and require separate explicit production approval; neither may be inferred from the already-applied `20260603142359_project_media_blocks.sql`.
  - Rollout order is fixed: approve/apply/read back expand migration A; freeze all Project editing before the separately approved authenticated preview writes; validate that workflow; promote the matching aggregate UI/endpoint to production; separately approve/apply/read back contract migration B and verify table/sequence privileges plus public policies; then lift the freeze. This continuous freeze prevents legacy child-table writes during the A/B overlap. Once B is applied, a Cloudflare-only rollback to the legacy direct-write UI is invalid. Runtime rollback must retain the aggregate UI/endpoint, or use a separately reviewed and approved forward-compatibility migration.
  ...
  - Stone Library, Products, and Articles still issue separate browser mutations rather than one database transaction. Projects is the source-only exception through the pending aggregate RPC; Storage promotion remains explicitly non-atomic and uses verification plus compensation. Partial failure must remain accurate in every module.
- Access control:
...
- Public Projects, Stone Library listing/detail, Products, and Articles use migration-safe Published overlays with static fallback in local source. Projects now has a source-only archived-slug tombstone path so Hide can suppress its matching static fallback after expand migration A and the aggregate runtime are deployed; other content types still require an explicit CMS-only cutover/removal policy.
...
- Admin editors outside Projects remain large schema-shaped components. Projects is now the source-only page-shaped aggregate prototype with one draft, one action bar, shared public/preview rendering, inline private media, and visual hotspots. Media discovery is still capped rather than paginated/virtualized, and neither deployment nor Jay's fool test has accepted the pattern yet.
...
- Project public detail consumes approved material/map/hotspot relationships in source and shares its renderer with draft preview. Do not call Projects end-to-end complete until the pending migration is approved/applied and one authenticated saved relationship is published, read back on the matching public route, hidden through the tombstone path, and accepted by Jay's fool test.
