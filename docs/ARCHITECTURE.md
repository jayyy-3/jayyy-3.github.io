...
- Cloudflare Pages repo-side clean URL configuration is in place. Production compute is now deployed in the Urblo-owned Cloudflare account `cb91e0806b808258d6d9c852c8d0f060` as Pages project `urblo-site` / `urblo-site.pages.dev`; `urblo.com.au` and `www.urblo.com.au` are attached and active. Current immutable deployment `https://42cabb15.urblo-site.pages.dev` is bound to `main` commit `004445b3b654ec6db0dbcd78a0a7c65b83665d09`, and both custom domains pass exact route, recursive asset byte/MIME, redirect, Function, and browser-secret-boundary comparison against it. The `.au` registry delegates the domain to Urblo-owned nameservers `mckenzie.ns.cloudflare.com` and `norman.ns.cloudflare.com`; the new Cloudflare zone is Active with 24 records, proxied apex/`www`, and the required Google Workspace, SMTP2GO, SendGrid, Squarespace, and `qa` records. Some recursive resolvers may temporarily return the old delegation until their cached NS TTL expires. Rollback boundaries are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
