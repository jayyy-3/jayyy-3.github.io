...
- Current implementation: Cloudflare Pages Function source now exists for `/api/enquiries`, `/api/sample-requests`, the protected CMS invite endpoint `/api/admin/invite-user`, the protected Projects aggregate endpoint `/api/admin/projects`, the protected Image QR endpoint `/api/admin/image-qr`, and the direct-image target `/image/:slug`.
...
- Stone Library current image status: Alpine White, Angola Black, Golden Crust Light/Dark, Honey Comb, Ivory Sand, Juparana, New Grey, Steel Blue, Tan Brown, and Zen Grey have finish-specific images. Tuscany Vein Cut and Cross Cut each expose only Honed and map that cut's approved image explicitly. Blueocean exposes its real Sawn image only for Sawn; other available finishes remain honest image-pending states and may not consume a group/default CMS media row. Harcourt still uses TBC placeholders because no matching current-site shared-drive sources were found.
...
- Project hotspot component: `src/components/projects/ProjectHotspotImage.tsx`
  ...
  - Hotspots are material-placement records keyed by `stoneGroupId`, `stoneVariantId`, and `finishKey`. Project owns the map image, point coordinates, application, and project note; Stone Library owns the material name, valid variant/finish relationship, finish preview image/alt, and detail link. Draft Project editing may select and preview non-Archived Stone Library rows so imported Draft content remains usable; Project publication still requires the selected Stone, Variant, Finish, finish-image link, and linked media asset to be Published. Public cards load only current Published Stone Library detail with static fallback, and Project-specific legacy title/image fields no longer override that canonical material presentation.
...
- Cloudflare Pages repo-side clean URL configuration is in place. Production compute is deployed in the Urblo-owned Cloudflare account `cb91e0806b808258d6d9c852c8d0f060` as Pages project `urblo-site` / `urblo-site.pages.dev`; `urblo.com.au` and `www.urblo.com.au` are attached and active. GitHub Actions now runs build, lint, type, smoke, and no-write Admin predeploy gates, deploys non-main branches as Preview, and deploys `main` as production with a minimal account-owned Pages Write token. Current immutable deployment `https://7e668551.urblo-site.pages.dev` is bound to `main` commit `aec4dce112873029de767239d7e7ee5ae88f7917`, and both custom domains pass exact route, recursive asset byte/MIME, redirect, Function, and browser-secret-boundary comparison against it. The zone uses `Full (strict)` SSL, has no custom WAF rules or Worker routes, and redirects HTTP origins to HTTPS. The `.au` registry delegates the domain to Urblo-owned nameservers `mckenzie.ns.cloudflare.com` and `norman.ns.cloudflare.com`; the new Cloudflare zone is Active with 24 records, proxied apex/`www`, and the required Google Workspace, SMTP2GO, SendGrid, Squarespace, and `qa` records. Some recursive resolvers may temporarily return the old delegation until their cached NS TTL expires. Rollback boundaries are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
