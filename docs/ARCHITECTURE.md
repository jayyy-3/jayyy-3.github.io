...
| `/admin/*` | `AdminApp` | Protected admin shell outside public site chrome. Config-gated until browser-safe Supabase key is set; uses Supabase Auth plus `admin_profiles` once configured. Current source CRUD/workflow screens: Settings, Media, Stone Library, Projects, Products, Articles, Leads. |
...
- Admin shell exists, with Settings, Media, Stone Library, Projects, Products, Articles, and Leads as the first source CRUD/workflow screens. Customer workflows for audit and admin-user management are not implemented yet.
