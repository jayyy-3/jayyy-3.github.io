...
- Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back, and the separately approved authenticated Preview happy path passed one Save/refresh, unsaved shared preview, inline media, visual hotspot, Publish/public readback, and Hide. That agent-run evidence still does not accept this Projects pattern. It remains a candidate until conflict/failure recovery is evidenced, the tombstone minimum-disclosure residual is narrowed or accepted, the aggregate runtime is promoted, contract migration `supabase/migrations/20260719015650_project_aggregate_write_lockdown.sql` is separately approved/applied/read back, and Jay passes the fool test. Source, migration, or agent-run browser gates cannot certify the editor experience, and the pattern must not be copied to other modules before that acceptance.
