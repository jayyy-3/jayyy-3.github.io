...
- Latest Projects uses `The work speaks.` as an image-led selected-project browser: a left-side active project narrative, a large active project image, and a horizontal thumbnail rail that shows four projects at a time on desktop. The whole section should occupy one viewport (`100svh`) and the active copy, image, and rail regions must keep fixed heights when hover, focus, or tap changes the selected project. Short desktop viewports may hide the active summary and place facts beside the CTA; short mobile viewports may simplify facts/summary so the section remains a clean one-screen browser. The upper `View project` CTA is the navigation path. Keep this as a restrained proof surface, not a grid of decorative project cards.
