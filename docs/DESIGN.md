...
- Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` and minimum-disclosure repair `supabase/migrations/20260802103337_restrict_archived_project_tombstones.sql` are applied/read back. The separately approved authenticated Preview happy path passed one Save/refresh, unsaved shared preview, inline media, visual hotspot, Publish/public readback, and Hide; deterministic source checks cover conflict reload-only locking and failed-Publish compensation; the fresh container gate and immutable Preview smoke/owner login also pass. That agent-run evidence still does not accept this Projects pattern. It remains a candidate until the aggregate runtime is promoted/read back, contract migration `supabase/migrations/20260802103338_project_aggregate_write_lockdown.sql` is separately approved/applied/read back, and Jay passes the fool test. Any real negative write proof needs separate approval. Source, migration, or agent-run browser gates cannot certify the editor experience, and the pattern must not be copied to other modules before that acceptance.
