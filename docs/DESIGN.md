...
- Latest Projects uses `The work speaks.` as an image-led selected-project browser: a left-side active project narrative, a large active project image, and a horizontal thumbnail rail that shows four projects at a time on desktop. Hover, focus, or tap may select a project; the upper `View project` CTA is the navigation path. Keep this as a restrained proof surface, not a grid of decorative project cards.
