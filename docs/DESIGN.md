...
- Project material points use Stone Library as their visual and naming authority: editors choose Stone, Variant, and only a supported Finish, then see the canonical finish image as a read-only preview. Project-specific controls are limited to placement, use, and project note; no separate point title or material/detail image is maintained in Projects.
