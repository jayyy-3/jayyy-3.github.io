...
- Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back, and the separately approved authenticated Preview happy path passed one Save/refresh, unsaved shared preview, inline media, visual hotspot, Publish/public readback, and Hide. The non-static QA marker proved public-not-found, not live suppression of a bundled fallback. Deterministic source checks now cover conflict reload-only locking and failed-Publish compensation. Source repair `supabase/migrations/20260719015650_restrict_archived_project_tombstones.sql` closes the minimum-disclosure defect but remains approval-gated and unapplied. That agent-run evidence still does not accept this Projects pattern. It remains a candidate until a fresh Preview, C and the aggregate runtime are promoted/read back, contract migration `supabase/migrations/20260719015651_project_aggregate_write_lockdown.sql` is separately approved/applied/read back, and Jay passes the fool test. Any real negative write proof needs separate approval. Source, migration, or agent-run browser gates cannot certify the editor experience, and the pattern must not be copied to other modules before that acceptance.
