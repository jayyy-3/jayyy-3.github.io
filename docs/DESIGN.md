...
- Image QR is a deliberately shallow admin tool: bulk drag-and-drop creates one resource per image, optimization progress is visible, and each card keeps preview, rename, replace, link copy, QR download, and Hide/Restore together. It must not inherit content-review or child publish-state complexity. The QR stays stable when its image changes, and scanning opens the image itself rather than an intermediate marketing page.
