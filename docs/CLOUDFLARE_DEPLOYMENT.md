...
Current environment identifiers and timestamped release observations are owned by `docs/agent/status.json`. The Urblo-owned `urblo-site` Pages project is the active delivery target; this document describes stable setup and rollback procedures. Earlier account checkpoints are preserved in `docs/archive/2026-09-09/docs__CLOUDFLARE_DEPLOYMENT.md`.
...
CI uses `--wait-for-functions` only on the newly deployed immutable Pages origin. It performs GET `/api/enquiries` for up to 120 seconds, requiring three consecutive exact 405 JSON `method_not_allowed` responses five seconds apart before the unchanged complete smoke runs. An intervening edge failure resets the consecutive count. HTML fallback/temporary edge errors and transport failures may wait; redirects, access/rate-limit responses and malformed API contracts fail immediately. Production alias readback never uses this wait. Every attempt records status, MIME, body classification and Cloudflare ray/error code without dumping response bodies; the smoke log is retained in CI artifacts. A readiness timeout or any subsequent strict smoke failure still blocks `quality`.
...
- Read the immutable runtime reference from `docs/agent/status.json` and bind apex and `www` smoke to it; verify the moving deployment when starting a new release.
...
- Output directory: `dist`
- Root directory: `/`
- Latest verified runtime observation: `docs/agent/status.json`
- Production URL: `https://urblo-site.pages.dev`
- Deployment status: `success`
- Runtime deployment commit: read the timestamped structured release observation.
...
- Production and Preview server Supabase bindings were configured/read back; Preview shares production data and maintenance QA is read-only. Local isolated tests own synthetic writes;
...
PRs always produce `quality`. Record-only changes verify without deploying; runtime changes deploy their branch and require immutable smoke. `main` runtime changes also require apex/www smoke bound to the deployment URL. Before that smoke, `scripts/wait-for-domain-convergence.mjs` polls apex and `www` read-only for up to 3 minutes until they serve the immutable asset graph byte-for-byte (edge lag caused the false failure in run 36153157216); the smoke then runs once, and the step fails if the domains never converge. Failure leaves the workflow red and prints the last recorded verified runtime and this rollback procedure. CI uses Node 20 for verification and Node 22 for pinned Wrangler 4.130.0 deployment. Local Docker remains Node 20. The stable check must be enabled by a GitHub administrator as a required main-branch PR check; code alone cannot provide branch protection.
