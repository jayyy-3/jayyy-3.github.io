...
It is both the schema design contract and the current implementation checkpoint record. The foundation migrations, baseline seeds, admin settings/profile/helper hardening, admin profile email uniqueness, media Storage role policies, Projects aggregate expand A, minimum-disclosure C, and write-lockdown B are applied/read back. Admin role helper RPC execution is revoked from browser roles in the exposed public schema, while RLS and Storage policies call private SECURITY DEFINER helpers from a non-exposed schema. The 2026-08-02 security advisor readback reports one Auth warning because leaked-password protection is disabled plus two expected warnings for the intentionally browser-callable archived-project tombstone RPC; B added no migration-specific warning. The aggregate Projects runtime is deployed and its production deployment-bound smoke/login pass. The first owner and a QA Editor exist, production browser configuration is present, Auth URL correction and the Media role boundary proof are complete, but Jay's Projects fool test, other authenticated UI golden workflows, repeated invite/recovery, and Turnstile remain open.
...
| Project aggregate migrations | Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` was applied/read back on 2026-07-19; minimum-disclosure C `supabase/migrations/20260802103337_restrict_archived_project_tombstones.sql` and contract B `supabase/migrations/20260802105537_project_aggregate_write_lockdown.sql` were applied/read back on 2026-08-02. B removed authenticated writes from all six Project tables/sequences and legacy mutation policies, retained service-role writes, left RLS enabled, and hardened public children behind approved Published parents. The aggregate runtime is deployed; fool-test acceptance remains open. |
...
- Expand A, minimum-disclosure C, and write-lockdown B are applied/read back, and the aggregate runtime is deployed. The approved authenticated Preview happy path, revision-conflict recovery, failed-Publish compensation checks, production deployment-bound smoke, and owner login pass. The edit freeze is lifted. Real two-session/Postgres/Storage negative proof requires fresh approval, and automated checks cannot pass Jay's fool test.
...
4. Projects and project material maps. The page-shaped aggregate runtime is deployed under `/admin/projects` and `/api/admin/projects`; expand A, minimum-disclosure C, write-lockdown B, authenticated Preview workflow, and production deployment checks pass. Optional approval-gated live negative proof and Jay's fool test remain pending.
...
The base relational tables, `20260603142359_project_media_blocks.sql`, expand A `20260719015649_project_aggregate_drafts.sql`, minimum-disclosure C `20260802103337_restrict_archived_project_tombstones.sql`, and write-lockdown B `20260802105537_project_aggregate_write_lockdown.sql` are applied/read back. The private draft, aggregate RPC, narrowed tombstone output, direct-write lockdown, and hardened public parent/child policies are production schema reality.
...
- The authenticated Preview happy path, deterministic conflict/compensation behavior, production runtime deployment, deployment-bound production smoke/login, and B readback are complete. The operational edit freeze is lifted. Real negative writes require separate approval.
- Migration B, `20260802105537_project_aggregate_write_lockdown.sql`, is the applied/read-back contract step. All six Project table and sequence privilege boundaries, public policies, RLS state, service-role write path, and security advisor state were verified on 2026-08-02.
