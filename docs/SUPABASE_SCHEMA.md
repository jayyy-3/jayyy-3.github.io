...
| Project aggregate migrations | Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` was approved, applied, and read back on 2026-07-19; a later approved Preview workflow left one tagged aggregate Archived. Production readback found A's public tombstone RPC returns four QA slugs including one never-published marker. Minimum-disclosure C `supabase/migrations/20260719015650_restrict_archived_project_tombstones.sql` and contract B `supabase/migrations/20260719015651_project_aggregate_write_lockdown.sql` remain source-only and separately approval-gated. Legacy table/sequence writes and policies remain until B; aggregate production runtime and fool-test acceptance do not exist yet. |
...
- Expand migration `20260719015649_project_aggregate_drafts.sql` is applied/read back, and the approved authenticated Preview happy path is complete. Source now covers revision-conflict reload-only recovery, mocked failed-Publish compensation, and minimum-disclosure C. Keep Project editing frozen; pass a fresh gate/Preview, separately approve/apply/read back C, separately approve/promote the aggregate runtime, then separately approve/apply/read back B and verify table/sequence privileges and public policies before lifting the freeze. Real two-session/Postgres/Storage negative proof requires fresh approval. Automated checks cannot pass Jay's fool test, so Phase 4d is not complete or deployed.
...
4. Projects and project material maps. The page-shaped aggregate source is implemented under `/admin/projects` and `/api/admin/projects`; expand A and the authenticated Preview happy path pass. A fresh Preview for the closeout source, C and B application/readback, aggregate production promotion, optional approval-gated live negative proof, and Jay's fool test remain pending.
...
The base relational tables, `20260603142359_project_media_blocks.sql`, and expand migration `20260719015649_project_aggregate_drafts.sql` are applied. The private draft, child-status/backfill, aggregate RPC, and A's over-broad archived-slug function are production schema reality. Minimum-disclosure C `20260719015650_restrict_archived_project_tombstones.sql` and final table/sequence/public-policy contract B `20260719015651_project_aggregate_write_lockdown.sql` remain source-only and each requires fresh explicit production approval/readback.
...
Behavior:
- Production A currently returns archived canonical and private-draft slugs; read-only proof found four QA slugs including one never-published marker. Source migration C replaces that query with only the intersection of archived canonical Projects and the five fallback slugs already public in `src/data/projectData.ts`, returns the allowlisted constant rather than stored raw text, and never reads `private.project_drafts`. `ProjectService` independently discards non-static results. C remains unapplied.
...
- Migration C, `20260719015650_restrict_archived_project_tombstones.sql`, is the minimum-disclosure repair. It is source-only and must receive separate approval/application/readback before production runtime promotion.
- The authenticated Preview happy path plus deterministic local conflict/compensation behavior are complete. Keep all Project editing frozen through a fresh gate/Preview, C, aggregate runtime production promotion, B application, and B readback; this prevents legacy child-table writes during the expand/contract overlap. Real negative writes require separate approval.
- Migration B, `20260719015651_project_aggregate_write_lockdown.sql`, is the contract step. Separately obtain Jay's approval, apply it only after the aggregate runtime is already in production, then read back all six Project table privileges, six sequence privileges, public policies, and the security advisor before lifting the edit freeze.
