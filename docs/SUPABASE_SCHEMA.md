...
| Project aggregate migrations | Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` was approved, applied, and read back on 2026-07-19: `private.project_drafts` is empty, child lifecycle backfill matches all parents, and existing Project counts/statuses are unchanged. Contract migration `supabase/migrations/20260719015650_project_aggregate_write_lockdown.sql` remains source-only and separately approval-gated; legacy table/sequence writes and policies remain until B. No tagged workflow, aggregate production runtime, or fool-test result exists yet. |
...
- Public Stone Library routes use Supabase published reads with static fallback; the listing can render published Supabase rows, while the detail page keeps the static variant/finish adapter until the deeper detail mapper is implemented.
...
- Expand migration `20260719015649_project_aggregate_drafts.sql` is applied/read back. Next, under a separate tagged-write approval, freeze Project editing and verify authenticated save/refresh/publish/public-readback/hide on the branch preview; resolve or accept the private-only archived-slug disclosure, promote the aggregate UI/endpoint to production, then separately approve/apply/read back contract migration `20260719015650_project_aggregate_write_lockdown.sql` and verify table/sequence privileges and public policies before lifting the freeze. Automated source or migration checks cannot pass Jay's fool test, so Phase 4d is not complete or deployed.
...
- Public Article routes use Supabase published reads with static fallback; article detail continues rendering sanitized legacy HTML through the legacy source path until structured public block rendering is implemented.
- Live browser save verification still requires browser-safe Supabase key configuration and an active admin/editor profile.
...
The base relational tables, `20260603142359_project_media_blocks.sql`, and expand migration `20260719015649_project_aggregate_drafts.sql` are applied. The private draft, child-status/backfill, aggregate RPC, and archived-slug entries below are production schema reality. Final table/sequence revocation and public parent/child policy hardening belong to contract migration `20260719015650_project_aggregate_write_lockdown.sql`, which remains source-only and requires a fresh explicit production approval.
