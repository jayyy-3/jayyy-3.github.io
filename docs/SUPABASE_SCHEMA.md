...
- `npm run agent:admin-crud-live` is now staged for that live proof. Default mode is no-write. With `--allow-writes`, it signs in or uses an admin access token and writes tagged QA rows across Settings, Media, Stone Library including finish images, Projects, Products, Articles, private lead workflow rows, and export-audit actions through authenticated RLS. It archives public-facing QA parents where possible, verifies tagged public-content QA rows are not anonymously visible, and performs no physical deletes.
