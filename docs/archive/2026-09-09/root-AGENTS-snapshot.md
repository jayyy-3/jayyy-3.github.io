...
- Projects aggregate expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql`, minimum-disclosure migration `supabase/migrations/20260802103337_restrict_archived_project_tombstones.sql`, and write-lockdown migration `supabase/migrations/20260802105537_project_aggregate_write_lockdown.sql` are applied/read back on production. Runtime merge `25c05ebb` is deployed as immutable Cloudflare deployment `877d13c4-1e28-45d7-a62a-afdd3b0e0dda`; the immutable URL, apex, and `www` passed deployment-bound smoke, and production passed all nine authenticated Admin routes. B removed authenticated direct writes to all six Project tables/sequences, removed legacy mutation policies, retained service-role writes, and hardened every public child read behind an approved Published parent. The operational edit freeze is lifted; Jay's fool test remains the acceptance and cannot be self-certified.
...
- Production Cloudflare compute and CI/CD are now in the Urblo-owned account (`cb91e0806b808258d6d9c852c8d0f060`) as Pages project `urblo-site`, default domain `urblo-site.pages.dev`. Current main `aec4dce112873029de767239d7e7ee5ae88f7917` is deployed at `https://7e668551.urblo-site.pages.dev`; both custom domains are attached, use `Full (strict)` SSL, and pass exact immutable-reference route/asset/MIME/redirect/Function smoke. GitHub Actions deploys branches as Preview and `main` as production with an account-owned Pages Write token. The `.au` registry delegates to Urblo-owned nameservers `mckenzie.ns.cloudflare.com` and `norman.ns.cloudflare.com`; the new zone is Active with all 24 required website, Google Workspace, SMTP2GO, SendGrid, Squarespace, and `qa` records. Google Public DNS sees the new pair, while some recursive caches may briefly retain the old delegation. The old Hunter-account project and zone were not deleted and are rollback evidence only. Rollback boundaries are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
...
- `/admin/projects` now uses the page-shaped aggregate editor and protected Function/RPC boundary in production. Its authenticated Preview happy path, production deployment-bound smoke/login, and A/C/B migration readbacks passed. Jay's fool test remains open, so this is deployed but not user-accepted.
