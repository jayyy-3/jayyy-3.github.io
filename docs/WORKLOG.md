...
- Added local migration source `supabase/migrations/20260818070944_image_qr_resources.sql`. The table is RLS-enabled and service-role-only; browser roles receive no direct table access. This migration has not been applied to production.
...
- Production is unchanged. No Supabase migration, database row, Storage object, Auth user, email, Cloudflare deployment, or custom-domain change occurred. Next boundary is Jay's separate approval before applying/read-back of the production migration.
