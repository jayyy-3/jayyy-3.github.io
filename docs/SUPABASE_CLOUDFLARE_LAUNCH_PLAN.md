...
- Static content import dry run is implemented through `scripts/check-content-import-readiness.mjs`; it prepares draft candidates, can emit a local ignored JSON artifact for review, and blocks missing local media or unknown stone/finish references before any apply step.
