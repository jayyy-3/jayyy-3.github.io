...
- Source-only static-to-Supabase import preparation now exists through `npm run agent:content-import`. It dry-runs current static Stone Library, Products, Projects, Articles, and media candidates into draft payloads without writing Supabase rows, and can write local ignored review, plan, preflight SQL, and guarded draft import SQL artifacts.
...
- Static content import dry run is implemented through `scripts/check-content-import-readiness.mjs`; it prepares draft candidates including Stone Library finish-image links, can emit local ignored JSON, Markdown plan, read-only preflight SQL, and guarded draft import SQL artifacts for review, and blocks missing local media or unknown stone/finish references before any approved apply step.
