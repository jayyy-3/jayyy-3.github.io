...
- Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back with an empty private draft table and unchanged existing Project counts/statuses. Contract migration `supabase/migrations/20260719015650_project_aggregate_write_lockdown.sql` later removes legacy browser writes and hardens public parent/child reads; it remains source-only and needs fresh approval. Before separately approved tagged preview writes, freeze Project editing and keep the freeze through workflow evidence, tombstone disclosure resolution or acceptance, aggregate runtime promotion, B approval/apply/readback, and privilege/policy verification. After B, do not Cloudflare-only roll back to the legacy direct-write UI. Until this sequence and Jay's fool test are complete, this is not a deployed or accepted Projects workflow.
