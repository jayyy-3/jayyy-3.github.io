...
| `/admin/image-qr` | Optimized image links and downloadable QR codes | Admin/editor write; viewer read | One uploaded image creates one stable Urblo image link and QR. Replacing the image keeps the link/QR; Hide pauses the link without destructive deletion. |
...
| `/admin/projects`, `/admin/projects/:projectId` | Project list and page-shaped case study editor | Admin/editor write; viewer read | Stable record URL; one aggregate draft with four expanded, collapsible sections (Hero and overview / Project information / Page images and video / Material schedule) and a sticky section bar; material points placed directly on any page image; one Save/preview/publish/hide action bar; searchable inline private media; actionable publish blockers. |
...
| Editor | Draft and edit content, manage non-sensitive media metadata. | Current RLS/UI can publish general content and already-public/external media. In Projects, the editor uses the same Save/Preview/Publish/Hide flow as owner/admin; legacy proof-review columns are normalized automatically and are not a separate approval step. The protected Projects service may promote referenced private uploads with server-side compensation even though Editors cannot promote files directly from the standalone Media screen. Editors still cannot manage users, delete records, or export leads. |
...
| Facts and claims | fact labels/values and display order | Required-content validation; no separate approval UI or review queue | claim taxonomy if later formalized |
...
| Material points on an image | Mark materials on this image, linked or newly added material, where it is used, point note, direct point placement and dragging | validated x/y percentage coordinates derived from the image; the hidden map record follows its image block | raw coordinate storage, map records and calibration rules |
...
- `/admin` dashboard reads content and lead counts after authentication, including a `Content health queue` for published media missing alt/usage notes, published Products/Articles missing key media, TBC Stone Library rows, and stale new leads older than 48 hours. Draft Projects can be continued directly; there is no claim-review queue.
...
- `/admin/projects` and `/admin/projects/:projectId` now have a Phase 1 page-shaped aggregate editor in source and on the authenticated branch Preview. The browser loads and submits the whole draft through authenticated `/api/admin/projects` GET/POST requests with private revision and canonical page-version tokens; one action bar owns Save, Publish, Hide, and a draft preview rendered by the same `ProjectPageView` component as the public route. The canonical token also protects the first Save of an existing Project that has no private draft yet. Hero and overview, Project information, Page images and video, and Material schedule are expanded collapsible sections with a sticky section bar showing counts and blocker dots; publish blockers expand and jump to the relevant section, and dirty guards cover record/route/refresh changes. Any page image can carry material points (the former separate Maps section is removed and older maps open as image blocks); turning points off or removing a points image asks for inline confirmation first. Facts, materials, media blocks and per-image points use accessible up/down ordering with no raw sort fields; the list/editor remain side by side around 1116px, while narrow section headers wrap their actions.
- Project Save automatically normalizes legacy project/fact/material review columns for compatibility. Editors do not see or operate proof-review decisions; complete content can be published without a second approver.
...
- Expand A, minimum-disclosure C, and contract B `supabase/migrations/20260802105537_project_aggregate_write_lockdown.sql` are applied/read back, and the aggregate runtime is deployed at production merge `25c05ebb`. Production deployment-bound smoke and all nine authenticated routes pass. The separately approved Preview workflow passed one Save/refresh, unsaved shared preview, private-first upload, visual hotspot, Publish/public readback, and Hide/public-not-found; deterministic checks cover stale-Save/reload-only recovery and failed-Publish compensation. The edit freeze is lifted, but Jay's fool test is still required before acceptance or reuse of this pattern. Any live negative write proof needs its own approval, and a Cloudflare-only rollback to the legacy direct-write UI remains invalid.
