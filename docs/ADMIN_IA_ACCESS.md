...
- Archive hides the CMS version without deleting history. Expand migration A installed an archived-slug tombstone read so a hidden Project can suppress its matching bundled fallback once the aggregate runtime is promoted; a tombstone read failure preserves the existing availability-first static fallback. Source repair `20260719015650_restrict_archived_project_tombstones.sql` restricts the RPC to archived canonical rows intersecting the five already-public fallback slugs and removes all private-draft reads. It remains unapplied and needs fresh production approval/readback before promotion.
...
- Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back. The separately approved production-backed preview workflow passed one Save/refresh, unsaved shared preview, private-first upload, visual hotspot, Publish/public readback, and Hide/public-not-found; its non-static QA slug did not prove the bundled-fallback tombstone path. Its tagged Project aggregate is retained Archived and its promoted Media/public object is retained Published under the archive-first policy. Deterministic local checks now cover stale-Save/reload-only recovery and failed-Publish compensation. Minimum-disclosure migration `supabase/migrations/20260719015650_restrict_archived_project_tombstones.sql` and contract migration `supabase/migrations/20260719015651_project_aggregate_write_lockdown.sql` are source-only and each needs fresh approval/readback. Keep Project editing frozen through a fresh Preview, C application, aggregate runtime promotion, B approval/apply/readback, and privilege/policy verification; any live negative write proof needs its own approval. After B, do not Cloudflare-only roll back to the legacy direct-write UI. Until this sequence and Jay's fool test are complete, this is not a production-deployed or accepted Projects workflow.
