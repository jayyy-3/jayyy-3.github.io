...
- `npm run agent:admin-auth-browser` is the gated no-write browser login check for the configured state. Default mode only prints required inputs; `--allow-login --strict` signs in with `URBLO_ADMIN_EMAIL` and `URBLO_ADMIN_PASSWORD`, verifies authenticated admin route shells, and creates no content rows, Storage objects, or audit events.
