...
| Projects | Edit case studies, project information, page images and video, material points, and the material schedule. | One page-shaped draft with Hero and overview / Project information / Page images and video / Material schedule, all open by default with a section bar that jumps to each part and marks what still blocks publishing. To show where a stone is used, click Mark materials on this image on any page image, click the image to add points, then choose or add the material for each point. One Save action, shared preview, inline media, Publish, and Hide. Inline uploads retain the full-quality original; the public website prepares high-quality card, detail, hero, and image versions automatically. There is no separate proof-review approval step. Jay's fool test remains the acceptance gate. |
...
| Image QR | Upload images that need a permanent scannable link. | Add one or many images; Urblo optimizes each one and creates its own QR. Download PNG/SVG, copy/open the QR page, rename, replace without changing the QR, or Hide/Restore the link. In the new material-page release, open a row → Stone & finish → select stone, variant/cut direction where available, and finish → check the surface preview → Save stone selection. Defaults need staff confirmation; saving changes that page while preserving the printed QR and product image. There is no review workflow. |
...
Before publishing:
- Projects need title, URL, public copy, valid referenced content, and the required images for the selected layout. Upload the best available source image: the CMS keeps that original at full quality and the website chooses a high-quality screen-sized version automatically. Save applies the whole page together. If Publish is blocked, use the named missing detail to jump to the right section; no second person or proof-approval state is required.
