...
- `NOW-ADMIN-AUTH-RLS-001` live verification: protected admin shell source exists, but active admin access still waits for first admin email/profile and browser-safe Supabase key configuration. Tagged admin write verification also waits for Jay approval plus a real owner/admin session before running `npm run agent:admin-crud-live -- --allow-writes`.
