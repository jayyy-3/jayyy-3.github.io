...
Homepage Latest Projects has a local release candidate that reads the complete Published Project collection in CMS order instead of the hand-maintained five-record list. The existing five records remain the no-CMS fallback, and the read is deferred until the section nears the viewport. Local desktop/mobile Browser QA reads the current 15 projects, reaches `15 / 15`, preserves the one-viewport rail composition, and has no horizontal page overflow. Next: run the clean gate, ship a Cloudflare Preview for Jay's fool test, then promote only after acceptance.
