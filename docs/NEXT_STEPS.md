...
The current `codex/homepage-logo-marquee` candidate adds the 12 supplied partner marks to the existing homepage white marquee, preserving the section composition while correcting the loop to two equal groups, hiding the duplicate accessibility copy, and adding a reduced-motion fallback. Local desktop/390px rendering and host runtime gates pass; commit, Preview smoke, and promotion wait for Jay's visual approval.
