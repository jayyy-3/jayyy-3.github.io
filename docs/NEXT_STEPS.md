...
- `NOW-PROJECT-STONE-POINTS-001`: local implementation makes Stone Library the canonical source for Project material-point stone/variant/finish names, imagery, and links; Project retains only map placement and project context. Read-only production inspection confirmed Moon Gate's Angola Black / Polished and New Grey / Flamed records, variants, capabilities, and images exist but are Draft. Editing therefore admits non-Archived Stone Library records while Project publication requires the full chain and media to be Live. Migration `20260812053204_project_materials_stone_library_source.sql` is prepared but unapplied; stop for migration approval after local/container verification.
