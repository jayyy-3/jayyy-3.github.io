...
- `NOW-PROJECT-STONE-POINTS-001`: local implementation makes Stone Library the canonical source for Project material-point stone/variant/finish names, imagery, and links; Project retains only map placement and project context. Migration `20260812053204_project_materials_stone_library_source.sql` is prepared but unapplied; stop for migration approval after local/container verification.
