...
- `NOW-PROJECT-STONE-POINTS-001`: local implementation makes Stone Library the canonical source for Project material-point stone/variant/finish names, imagery, and links; Project retains only map placement and project context. Production migration `20260812062636` is applied/read back and uniquely links Moon Gate's Draft Angola Black / Polished and New Grey / Flamed records. Editing admits non-Archived Stone Library records while Project publication requires the full chain and media to be Live. Next stop is separate production runtime-promotion approval for draft PR `#23`, followed by Jay's fool test.
