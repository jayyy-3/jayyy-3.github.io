...
- `NOW-ADMIN-CONTENT-CRUD-001`: continue after the Projects checkpoint. `/admin/stone-library` and `/admin/projects` source CRUD are implemented; live save verification and static-to-Supabase content import still require browser-safe Supabase key configuration and an active admin/editor profile. The next source-only checkpoint is Products.
