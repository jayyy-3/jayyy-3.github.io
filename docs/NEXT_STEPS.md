...
Draft PR `#11` simplifies the Project editor and Dashboard to a single-editor Edit/Save/Preview/Publish/Hide workflow: legacy claim-review columns are normalized automatically and no longer appear as approval controls or Dashboard queues. Its first public-header repair made the translucent header render over the white Projects page, but Jay rejected that visual result. The current correction gives Projects listing/detail the same 102px black `DefaultLayout` support band as Stone Library and removes the duplicate page-local offset; a fresh immutable Preview is required before production promotion can be requested again.
...
- `NOW-ADMIN-UX-RESHAPE-001`: Phase 0 is closed and the Projects aggregate draft, one-action-bar, shared-preview, visual-hotspot, inline-media, and protected aggregate runtime is deployed. The clean Node 20 container gate, deterministic conflict/compensation checks, Preview and production smoke/login, A/C/B migrations, and separately approved authenticated Preview happy path pass. Keep the task active through Jay's fool test and any desired approval-gated live negative proof.
  - Current follow-up: align the public Projects header treatment exactly with Stone Library and remove Project claim-review UI/queues without weakening required-field, media, auth, RLS, promotion, conflict, or audit boundaries.
