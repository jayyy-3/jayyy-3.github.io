...
- Live input readiness: `npm run agent:live-readiness` reports whether the required form, admin, admin-write, and Cloudflare preview live verification inputs are present, without printing secrets or mutating Supabase/Cloudflare. Use `--strict` only when missing/manual-gated live inputs should fail the command.
