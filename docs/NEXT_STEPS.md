...
The current Project image-delivery candidate preserves every CMS upload as the original visual master and gives the public archive, list, hero, detail, and hotspot surfaces separate responsive Supabase WebP profiles. Local Browser QA confirms desktop cards choose a 960px quality-82 image and the 120px list view chooses a 240px image. Draft PR `#24` commit `9209d65` deployed as immutable Preview `f9122a0d-7bfa-44b4-ab5a-bd229f8f35f3`; route/asset/redirect/Function smoke passes, the 1280px Preview hero chooses the 2500px quality-88 profile, and a 390px Preview hero chooses 960px without horizontal overflow or console warnings/errors. The Admin upload surface reassures editors that the original remains full quality and avoids presenting byte reduction as the outcome. Complete host and clean Node 20 container gates pass; Jay's visual acceptance remains before production promotion.
