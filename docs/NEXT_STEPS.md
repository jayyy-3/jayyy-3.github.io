...
Measured 2026-05-28 during the Leads CSV export source checkpoint:
- `npm run build`: pass
- `npm run lint`: pass
- `npx tsc -b`: pass
...
- `NOW-ADMIN-MEDIA-LEADS-001`: continue after the Leads checkpoint. Supabase Storage buckets/policies, `/admin/media`, and `/admin/leads` source are implemented; Leads now has an audit-gated owner/admin CSV export path for the currently loaded queue. Live upload/save/export and lead workflow verification still require browser-safe Supabase key configuration and active admin profiles. Live lead creation still requires server-side form persistence verification.
