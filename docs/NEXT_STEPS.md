...
Jay approved Preview connection setup, the additive field, save/readback verification and promotion after passing gates on 2026-09-09. Migration `20260909054816_image_qr_material_selection.sql` is applied and read back: nullable JSONB column and shape constraint exist, RLS remains enabled, anonymous SELECT and authenticated direct UPDATE remain denied, and all 32 rows retain null selections. The local filename now matches the applied migration history. Preview now has encrypted `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_URL` bindings, set with official Wrangler single-variable commands after Jay explicitly approved existing-key retrieval. Readback confirms production configuration and all other Preview settings are unchanged; temporary key files were removed. Redeploy with the bindings, verify the same-material Zen Grey/Honed UI save/refresh and public readback, then promote only after all gates pass. Staff still own default confirmation.
...
The public-catalog repair is live through PR `#36` / merge `5731651` / immutable Cloudflare production `https://764dedbc.urblo-site.pages.dev`: Homepage has WeChat-tolerant playback with truthful tap recovery, all six Product families use reviewed model/render mappings, Stone Finish changes preserve reading position, Tuscany uses a right-side `Cut direction → Finish` rail and Honed-only truth, and one canonical BlueOcean uses the complete former Steel Blue record/image set while the duplicate public record is removed. Immutable/apex/`www` smoke and desktop/390px production Browser checks pass. Real-device WeChat confirmation and separately approved CMS Draft synchronization/duplicate-row merge remain.
...
The 2026-09-02 Homepage Acknowledge follow-up is live through PR `#34` / merge `e298d06` / immutable production deployment `737e702b`. The first-visit panel now uses the accepted Homepage overlay material (`bg-black/50` plus the same 4px light blur), and smoke guards against the legacy `/88` black block. Local desktop/390px QA, the clean Node 20 gate, Jay's explicit Preview acceptance, and immutable/apex/`www` deployment-bound smoke pass.
...
- Phase 1 SEO indexability foundation is implemented in source: real `robots.txt`, real `sitemap.xml` with 35 approved public URLs after the Steel Blue/BlueOcean deduplication, centralized public route metadata in `src/data/seoRoutes.ts`, conservative client-side JSON-LD, and `npm run agent:seo-readiness`.
...
- `NOW-PUBLIC-CATALOG-INTEGRITY-001`: implement and Preview the Homepage WeChat video, Product model imagery, Stone detail truth/interaction repairs, and Steel Blue → BlueOcean canonical merge. Source writes are local only; do not update or delete Supabase Draft rows without Jay's separate approval.
