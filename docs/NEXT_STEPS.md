...
- Content import artifact checkpoint: the dry run now supports `--out` for a local ignored JSON review artifact, and `.tmp/` is ignored so review payloads are not accidentally committed.
