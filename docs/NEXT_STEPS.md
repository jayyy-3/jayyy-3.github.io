...
## Current Objective
Urblo is operating as a Cloudflare Pages + Supabase site with real forms, but the production `/admin` handoff remains reopened after a direct not-working report and a 2026-07-13 code/UX audit. PR `#9` promoted the page-shaped Projects Phase 1 runtime to production at merge `25c05ebb` / immutable deployment `877d13c4-1e28-45d7-a62a-afdd3b0e0dda`; deployment-bound smoke passed on the immutable URL, apex, and `www`, and production owner login passed all nine Admin routes. Expand A, minimum-disclosure C, and write-lockdown B are applied/read back. The operational edit freeze is lifted; Jay-owned fool-test acceptance and any separately approved optional live negative proof remain open.

Draft PR `#11` fixes the public Projects/navigation transparency regression by generating every integer opacity modifier used by the UI and checking critical navbar/menu/detail utilities in the built CSS. It also simplifies the Project editor and Dashboard to a single-editor Edit/Save/Preview/Publish/Hide workflow: legacy claim-review columns are normalized automatically and no longer appear as approval controls or Dashboard queues. Implementation commit `610d4b2` passed the clean container gate; immutable Preview `https://3b285f72.urblo.pages.dev` passed no-write smoke, nine-route owner login, and visible navbar/menu opacity readback. Production promotion still requires approval.
...
- `NOW-ADMIN-UX-RESHAPE-001`: Phase 0 is closed and the Projects aggregate draft, one-action-bar, shared-preview, visual-hotspot, inline-media, and protected aggregate runtime is deployed. The clean Node 20 container gate, deterministic conflict/compensation checks, Preview and production smoke/login, A/C/B migrations, and separately approved authenticated Preview happy path pass. Keep the task active through Jay's fool test and any desired approval-gated live negative proof.
  - Current follow-up: fix the public opacity regression and remove Project claim-review UI/queues without weakening required-field, media, auth, RLS, promotion, conflict, or audit boundaries.
...
- Run Jay's production fool test. Contract migration `supabase/migrations/20260802105537_project_aggregate_write_lockdown.sql` is applied/read back, and the edit freeze is lifted. Never Cloudflare-only roll back to the legacy direct-write UI. Only Jay may accept the fool test.
