...
- Phase 0 of the approved Admin reshape is complete. Phase 1 Projects is implemented in local source with one aggregate draft/action bar, shared public/draft rendering, visual hotspots, inline private-first media, a protected server aggregate path, and source-level security hardening. Expand migration A is applied/read back, but the UI remains a branch-preview milestone rather than production or user-accepted evidence.
...
- `NOW-ADMIN-UX-RESHAPE-001`: Phase 0 is closed and the Projects aggregate draft, one-action-bar, shared-preview, visual-hotspot, inline-media, and protected aggregate source candidate is implemented. The full host-side local suite and deployment-bound no-write preview smoke pass; Docker is unavailable and Jay approved a one-time exception for the original branch push. Expand migration `20260719015649_project_aggregate_drafts.sql` is applied/read back. Keep the task active until the separately approved authenticated preview workflow passes, the tombstone disclosure is narrowed or accepted, the aggregate runtime is promoted, contract migration `20260719015650_project_aggregate_write_lockdown.sql` is separately approved/applied/read back, and Jay completes the fool test.
...
- Expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied and read back with zero private drafts and unchanged existing Project/child counts. Latest deployed preview commit `9441811` serves at `https://d29d45cf.urblo.pages.dev` and passes the no-write route/asset/API smoke; GitHub's Cloudflare callback lagged at `in_progress` after the deployment was independently verified.
...
- Before production runtime promotion, narrow or explicitly accept the low-risk tombstone case where a never-published archived draft could expose its slug; the current private draft table is empty.
...
The 2026-07-14 Projects Phase 1 source candidate is newer than the production runtime proof set. Its host-side local suite, read-only 1116px/mobile implementation check, and immutable no-write branch-preview smoke pass. Docker was unavailable; Jay approved a one-time container-gate exception for the original push on 2026-07-16. Expand migration A is applied/read back; authenticated workflow, runtime promotion, contract B/privilege-policy readback, and fool test remain pending. It must not inherit the 2026-07-13 production status.
