...
The 2026-09-02 Homepage Acknowledge follow-up restores that first-visit panel to the accepted Homepage overlay material (`bg-black/50` plus the same 4px light blur). Its legacy `/88` class only became visibly near-opaque after Tailwind's opacity scale expanded; the source smoke now guards this coupling. Local desktop/390px QA, the clean Node 20 gate, and immutable Preview smoke pass; Jay explicitly accepted the Preview with `没问题`, so PR `#34` is approved for production promotion and final production-bound smoke.
