...
- Push the Phase 1 Projects candidate under Jay's one-time Docker-gate exception and run the branch preview's no-write route/asset/API smoke. Then obtain explicit approval before applying expand migration `supabase/migrations/20260714052955_project_aggregate_drafts.sql`; it is source-only, unapplied, and its lifecycle backfill is a production write.
...
The 2026-07-14 Projects Phase 1 source candidate is newer than that deployed proof set. Its host-side local suite and read-only 1116px/mobile implementation check pass. Docker was unavailable; Jay approved a one-time container-gate exception for this push on 2026-07-16. The preview gate, unapplied expand/contract migrations, authenticated workflow, privilege/policy readback, and fool test remain pending. It must not inherit the 2026-07-13 deployment status.
