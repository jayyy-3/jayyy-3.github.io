...
- `NOW-ADMIN-AUTH-RLS-001` partial source implementation: `/admin`, `/admin/login`, `/admin/unauthorized`, protected admin module scaffolds, Supabase Auth session/profile checks, config-required state, dashboard query shell, and current launch-check copy are implemented. Live active-admin verification remains pending first admin email/profile and browser-safe Supabase key configuration.
