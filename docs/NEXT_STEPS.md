...
- Admin live write verification: `npm run agent:admin-crud-live` now prints a no-write plan by default. Use `npm run agent:admin-crud-live -- --allow-writes` only after credentials/session exist and tagged QA writes are approved; it will also verify tagged public-content QA rows are not anonymously visible. Use `--include-storage` when intentionally verifying private Storage upload policy.
