...
- `NOW-ADMIN-UX-RESHAPE-001`: Phase 0 is closed and the Projects aggregate draft, one-action-bar, shared-preview, visual-hotspot, inline-media, and protected aggregate source candidate is implemented. The full host-side local suite and deployment-bound no-write preview smoke pass; Docker is unavailable and Jay approved a one-time exception for this branch push only. Keep it active until expand migration `20260714052955_project_aggregate_drafts.sql` is separately approved/applied/read back, the separately approved authenticated preview workflow passes, the aggregate runtime is promoted, contract migration `20260714052956_project_aggregate_write_lockdown.sql` is separately approved/applied/read back, and Jay completes the fool test.
...
- The Phase 1 branch is pushed and immutable preview `https://1c3372dd.urblo.pages.dev` passes the no-write route/asset/API smoke for commit `30e9b57`; GitHub's Cloudflare callback lagged at `in_progress` after the deployment was already independently verified. Obtain explicit approval before applying expand migration `supabase/migrations/20260714052955_project_aggregate_drafts.sql`; it is source-only, unapplied, and its lifecycle backfill is a production write.
...
The 2026-07-14 Projects Phase 1 source candidate is newer than the production proof set. Its host-side local suite, read-only 1116px/mobile implementation check, and immutable no-write branch-preview smoke pass. Docker was unavailable; Jay approved a one-time container-gate exception for this push on 2026-07-16. The unapplied expand/contract migrations, authenticated workflow, privilege/policy readback, and fool test remain pending. It must not inherit the 2026-07-13 production status.
