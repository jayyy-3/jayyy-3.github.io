...
Image QR is now a Previewed source candidate under the active Admin reshape: `/admin/image-qr` bulk-optimizes images, creates one stable direct-image QR per file, supports PNG/SVG download, rename, replace-without-changing-the-code, and Hide/Restore, and keeps service-role/database/Storage operations behind protected Functions. Commit `842c11e` passes the clean Node 20 container gate. Separately approved migration `20260818074046_image_qr_resources.sql` is applied/read back with zero resource rows. Draft PR `#29` immutable Preview `https://35044c9d.urblo.pages.dev` passes full no-write smoke. Tagged upload/replace proof and production runtime promotion remain separately approval-gated before Jay's fool test.
...
- Obtain separate approval before any tagged Image QR upload/replace/Hide/Restore proof; the gated source, applied migration, and passing Preview smoke do not substitute for Jay's final usability test or authorize production runtime promotion.
