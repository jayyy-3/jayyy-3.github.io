...
- `NOW-FORMS-BACKEND-001` live verification runner checkpoint: `npm run agent:forms-live` now verifies valid enquiry/sample request rows, sample item rows, server-side audit rows, invalid-payload no-write behavior, and response-vs-stored `notification_status` consistency once a service-role key is configured. The runner intentionally fails without credentials and retains tagged test rows until Jay approves cleanup.
