...
- `NOW-ADMIN-UX-RESHAPE-001`: Phase 0 is closed and the Projects aggregate draft, one-action-bar, shared-preview, visual-hotspot, inline-media, and protected aggregate source candidate is implemented. The full host-side suite, deployment-bound no-write smoke, expand migration A, and separately approved authenticated Preview happy path pass; Docker is unavailable and Jay approved a one-time exception for the original branch push. Keep the task active until conflict/failure recovery is evidenced, the tombstone disclosure is narrowed or accepted, the aggregate runtime is promoted, contract migration `20260719015650_project_aggregate_write_lockdown.sql` is separately approved/applied/read back, and Jay completes the fool test.
...
- Keep the Project edit freeze through remaining conflict/failure-recovery evidence, production promotion, and contract readback. Promote the aggregate runtime only after Preview evidence and the tombstone decision are acceptable.
- Before production runtime promotion, narrow or explicitly accept the low-risk tombstone case where a never-published archived draft could expose its slug. The current private aggregate is a previously published archived QA Project, not the private-only case.
