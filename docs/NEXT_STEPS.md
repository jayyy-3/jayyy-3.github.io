...
- `NOW-ADMIN-CONTENT-CRUD-001`: continue after the Stone Library checkpoint. `/admin/stone-library` source CRUD is implemented; live save verification and static-to-Supabase content import still require browser-safe Supabase key configuration and an active admin/editor profile. The next source-only checkpoint is Projects with material maps/hotspots.
