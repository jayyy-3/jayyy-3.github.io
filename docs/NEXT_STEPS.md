...
The current Project image-delivery candidate preserves every CMS upload as the original visual master and gives the public archive, list, hero, detail, and hotspot surfaces separate responsive Supabase WebP profiles. Local Browser QA confirms desktop cards choose a 960px quality-82 image and the 120px list view chooses a 240px image. Draft PR `#24` includes Grid black-bar repair `c1920e1`, deployed as immutable Preview `aa70ae54-2d3b-4440-bf11-799114226fec`; route/asset/redirect/Function smoke passes. Desktop Grid images now exactly match their 348×261 frames, List remains 120×90, and the 390px Grid remains 342×257 without horizontal overflow or console warnings/errors. Detail heroes retain their separately verified 2500px/960px quality-88 responsive selection. The Admin upload surface reassures editors that the original remains full quality and avoids presenting byte reduction as the outcome. Complete host and clean Node 20 container gates pass; Jay's visual acceptance remains before production promotion.
