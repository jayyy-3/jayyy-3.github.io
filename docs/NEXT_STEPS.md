...
## Current Objective
Urblo is operating as a Cloudflare Pages + Supabase site with real forms, but the production `/admin` handoff is reopened after a direct not-working report and a 2026-07-13 code/UX audit. PR `#6` restored production asset integrity and the no-write auth/fallback baseline after PR `#5` exposed a cached-asset incident that the old status-only smoke had falsely accepted. Jay approved `docs/ADMIN_UX_RESHAPE_PLAN.md` on 2026-07-14: Phase 0 is closed, the page-shaped Projects Phase 1 source candidate, no-write branch preview, expand migration A, and authenticated Preview happy path pass. Local deterministic conflict/compensation checks and a minimum-disclosure migration C are now implemented; a fresh gated Preview, C application/readback, aggregate runtime promotion, contract migration B, optional separately approved live negative proof, and Jay-owned fool test remain open.
...
- Phase 0 of the approved Admin reshape is complete. Phase 1 Projects is implemented in source and its authenticated branch Preview with one aggregate draft/action bar, shared public/draft rendering, visual hotspots, inline private-first media, a protected server aggregate path, and source-level security hardening. Expand migration A and the approved owner happy path pass, but the UI remains a branch-preview milestone rather than production or user-accepted evidence.
...
- `NOW-ADMIN-UX-RESHAPE-001`: Phase 0 is closed and the Projects aggregate draft, one-action-bar, shared-preview, visual-hotspot, inline-media, and protected aggregate source candidate is implemented. The host-side suite, deterministic conflict/compensation checks, deployment-bound no-write smoke, expand migration A, and separately approved authenticated Preview happy path pass; Docker is unavailable and Jay approved a one-time exception for the original branch push only. Keep the task active through a fresh delivery gate/Preview, separately approved C application/readback, aggregate runtime promotion, separately approved contract migration `20260719015651_project_aggregate_write_lockdown.sql`, any desired approval-gated live negative proof, and Jay's fool test.
...
- Keep the Project edit freeze through the fresh Preview, C application/readback, production promotion, and contract readback. Local stale-Save and failed-Publish compensation behavior is covered; real two-session/Postgres/Storage negative proof needs its own approval if requested.
- The tombstone choice is resolved in source: `20260719015650_restrict_archived_project_tombstones.sql` returns only archived canonical rows whose slug is one of the five already-public bundled fallbacks, and the client independently rejects any other result. Production A still returns four QA slugs including one never-published marker, so obtain fresh approval and apply/read back C before promotion.
...
The 2026-07-14 Projects Phase 1 source candidate is newer than the production runtime proof set. Its host-side local suite, read-only 1116px/mobile implementation check, immutable no-write branch-preview smoke, expand migration A, and approved authenticated Preview happy path pass. Local deterministic stale-Save, conflict-lock, and failed-Publish compensation checks plus minimum-disclosure C are newer still. Docker was unavailable; Jay approved a one-time container-gate exception for the original push on 2026-07-16, not this follow-up. A fresh gated Preview, C application/readback, runtime promotion, contract B/privilege-policy readback, any separately approved live negative proof, and fool test remain pending. Preview evidence must not inherit the 2026-07-13 production status.
