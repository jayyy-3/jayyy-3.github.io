...
test('Function entry points: OPTIONS 204, unsupported method 405, no session 401 and blank public slug 404', async () => {
...
test('default material resolution matches the database catalogue and hides managed stones', async () => {
...
test('public QR page renders safe embedded metadata without mutation; HEAD is empty and POST is 405', async () => {
...
test('material assignment enforces role, validation and stale-save guards; save, rename, hide and restore keep the image and selection', async () => {
