...
test('Project Grid cards keep a 4:3 media frame and intrinsic image row', () => {
...
test('Published records replace matching static records without hiding unmatched ones', () => {
...
test('public media and Project image delivery reject unsafe sources and draft media', () => {
...
test('SEO, Project fact values and article destinations are normalized safely', () => {
...
test('Published CMS Projects map, overlay static fallbacks, honour tombstones and resolve Stone reverse references', async () => {
...
test('tombstone or child-table read failures keep static Project fallbacks intact', async () => {
...
test('Published site settings normalize consistently and internal footer links stay same-origin', () => {
...
test('site settings loader shares in-flight reads, retries after failure and refreshes on next mount', async () => {
...
test('source fragments keep the protected overlay, SEO and settings contracts wired', () => {
  const requiredSourceFragments = new Map([
    ...
    ['src/components/PublicContentSeo.tsx', ['upsertDynamicJsonLd', 'buildPublicContentSeoMeta', "tag?.dataset.owner === 'public-content-seo'"]],
    ['src/lib/publicContentSeoMeta.ts', ["'@type': 'Article'", "'@type': 'WebPage'", "'@type': 'BreadcrumbList'", 'getStoneShareImageUrl']],
    ...
    ['src/pages/admin/articles/useArticleEditor.ts', ['blockLoadGenerationRef']],
    ['src/pages/admin/articles/data.ts', [".eq('article_id', articleId)"]],
    ['src/pages/admin/articles/forms.ts', ['toSafePublicContentDestination']],
    ['src/pages/ArticlePage.tsx', ['toSafePublicContentDestination', 'destination?.kind === \'internal\'']],
    ['src/service/StoneLibraryService.ts', ['getPublicStoneCards', 'const excluded = new Set([...catalogue.managedKeys', "rpc('public_stone_catalogue')"]],
    ['src/pages/StoneLibraryPage.tsx', ['getFilterFacets(publicCards)', 'filterStoneCards(publicCards']],
    ['src/pages/StonePageView.tsx', ['<PublicContentSeo', "contentSource === 'cms'"]],
...
test('company address edits preserve other footer content and feed every consumer', () => {
...
test('Stone media renders through sized variants; the original is reserved for View original', async () => {
  const { STONE_RENDER_MAX_WIDTH, getStoneImageDelivery, getStoneShareImageUrl, stoneImageProfiles } = await import(
    '../src/lib/stoneImageDelivery.ts'
  );
  ...
  assert.match(
    getStoneImageDelivery(stoneStorageImage, 'zoom').srcSet ?? '',
    new RegExp(`width=${STONE_RENDER_MAX_WIDTH}&quality=90&format=webp.* ${STONE_RENDER_MAX_WIDTH}w`),
  );
  assert.ok(
  ...
  assert.ok(
    stageSource.includes('View original') && stageSource.includes('href={activeFinish.imageUrl}'),
    'Stone detail stage keeps an explicit View original link to the untouched source',
  );
