...
test('sample request atomic migration keeps the RPC security-invoker and service-role only', () => {
...
test('GET is rejected with 405 before any Supabase call', async () => {
...
test('OPTIONS advertises POST and content-type without any Supabase call', async () => {
  await withFetchMock(async (calls) => {
    for (const [path, onRequest] of [
      ['/api/enquiries', onEnquiryRequest],
      ['/api/sample-requests', onSampleRequest],
...
test('empty enquiry fails validation before any Supabase call', async () => {
  await withFetchMock(async (calls) => {
...
test('invalid sample request fails validation before any Supabase call', async () => {
  await withFetchMock(async (calls) => {
...
test('malformed JSON returns invalid_json before any Supabase call', async () => {
  await withFetchMock(async (calls) => {
    for (const [path, onRequest] of [
      ['/api/enquiries', onEnquiryRequest],
      ['/api/sample-requests', onSampleRequest],
...
test('valid enquiry inserts the row and records an audit event', async () => {
  await withFetchMock(async (calls) => {
...
test('valid sample request uses the atomic RPC and records an audit event', async () => {
  await withFetchMock(async (calls) => {
...
test('Resend notification success marks the enquiry sent', async () => {
  await withFetchMock(async (calls) => {
    const response = await handleEnquiryRequest(
      jsonRequest('/api/enquiries', {
        name: 'Notify Success',
...
test('SMTP2GO is preferred over Resend and marks the enquiry sent', async () => {
  await withFetchMock(async (calls) => {
    const response = await handleEnquiryRequest(
      jsonRequest('/api/enquiries', {
...
test('SMTP2GO rejection marks the sample request notification failed', async () => {
  await withFetchMock(async (calls) => {
    const response = await handleSampleRequest(
      jsonRequest('/api/sample-requests', {
...
test('Resend failure marks the sample request notification failed', async () => {
  await withFetchMock(async (calls) => {
    const response = await handleSampleRequest(
      jsonRequest('/api/sample-requests', {
        name: 'Notify Failure',
...
test('audit logging failure does not fail the visitor response', async () => {
  await withFetchMock(async (calls) => {
    const response = await handleEnquiryRequest(
      jsonRequest('/api/enquiries', {
...
test('missing service-role configuration fails closed before any Supabase call', async () => {
  await withFetchMock(async (calls) => {
    const response = await handleEnquiryRequest(
      jsonRequest('/api/enquiries', {
...
test('failed Turnstile verification rejects the enquiry before any insert', async () => {
  await withFetchMock(async (calls) => {
    const response = await handleEnquiryRequest(
      jsonRequest('/api/enquiries', {
        name: 'Sam Reviewer',
