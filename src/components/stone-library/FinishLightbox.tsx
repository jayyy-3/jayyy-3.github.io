...
export default function FinishLightbox({
    ...
    const finishFrames = activeFinish
        ? [
              ...(activeFinish.imageUrl
                  ? [
                        {
                            imageUrl: activeFinish.imageUrl,
                            thumbUrl: activeFinish.thumbUrl,
                            imageAlt: activeFinish.imageAlt,
                            label: 'Primary frame',
                        },
                    ]
                  : []),
              ...activeFinish.secondaryImages,
          ]
        : [];
    const activeFrameIndex = finishFrames.length
        ? clamp(frameIndex, 0, finishFrames.length - 1)
        : 0;
    const activeFrame = finishFrames[activeFrameIndex] || null;
    const hasMultipleFrames = finishFrames.length > 1;
    ...
    useEffect(() => {
        ...
        resetZoom();
    }, [isOpen, currentIndex, activeFrameIndex]);
    ...
    }, [activeFinishKey, initialFrameIndex, isOpen]);
    ...
    return (
        ...
        >
            <div className="mx-auto flex h-full w-full max-w-7xl flex-col px-4 py-4">
                ...
                {hasMultipleFrames ? (
                    <div className="mt-3 flex gap-2 overflow-x-auto rounded-[4px] border border-white/15 bg-black/42 p-2">
                        {finishFrames.map((frame, index) => {
                            const isActiveFrame = index === activeFrameIndex;

                            return (
                                <button
                                    key={`${activeFinish.finishKey}-${frame.imageUrl}`}
                                    type="button"
                                    onClick={() => setFrameIndex(index)}
                                    className={[
                                        'flex w-[118px] flex-none flex-col overflow-hidden rounded-[4px] border text-left transition',
                                        isActiveFrame
                                            ? 'border-[#00FF19] bg-white text-black'
