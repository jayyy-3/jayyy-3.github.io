...
- `20260802103338_project_aggregate_write_lockdown.sql` — contract step that removes legacy browser Project table/sequence writes and hardens public parent/child reads. Apply only with separate approval after preview passes and the aggregate UI/endpoint is already promoted to production; freeze Project editing during apply/readback. After this step, do not Cloudflare-only roll back to the legacy direct-write UI.

Migration A is already applied and remains byte-for-byte immutable. Its historical comment names the originally planned contract step; production-history alignment inserted C at recorded version `20260802103337` and moved the still-unapplied contract step to `20260802103338`.
