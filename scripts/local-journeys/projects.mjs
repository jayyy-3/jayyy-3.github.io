...
export async function projectJourney({ page, context, check, id, directory }) {
  ...
  await check('Projects Publish exposes approved parent and Hide removes it', async () => {
    ...
    // NOW-OPT-ADMIN-SAFETY-001: saving a newer private draft of a live Project keeps the public
    // page unchanged, and the editor says it is still live rather than "Saved — not live".
    await page.getByRole('textbox', { name: 'Opening line', exact: true }).fill('Synthetic live project edit waiting to publish.')
    const edited = await action('Save')
    assert.equal(edited.status, 'draft')
    const bar = page.getByTestId('project-sticky-action-bar')
    await expect(bar.getByText('Live · unpublished changes', { exact: true })).toBeVisible()
    await expect(page.getByText('Saved — not live', { exact: true })).toHaveCount(0)
    await expect(bar.getByRole('link', { name: 'Open live page', exact: true })).toHaveAttribute('href', `/projects/${published.draft.project.slug}`)
    await expect(page.getByRole('button', { name: new RegExp(`${title}.*Live · changes`) })).toBeVisible()
    const stillLive = await publicRows(published.draft.project.slug)
    assert.equal(stillLive.length, 1); assert.equal(stillLive[0].title, title)
    await page.screenshot({ path: `${directory}/project-live-unpublished-changes.png`, fullPage: true })
    await action('Hide')
