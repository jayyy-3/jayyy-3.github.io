...
function buildPreflightSql(payload) {
    ...
    return [
        ...
        "  has_table_privilege('service_role', format('public.%I', table_name), 'insert') as service_role_insert,",
        "  has_table_privilege('service_role', format('public.%I', table_name), 'update') as service_role_update,",
        "  has_table_privilege('service_role', format('public.%I', table_name), 'delete') as service_role_delete",
        `from unnest(${tableArray}) as expected(table_name)`,
        'order by table_name;',
        '',
        '-- Data API sequence grants for generated identity rows.',
        "select n.nspname as sequence_schema, c.relname as sequence_name,",
        "  has_sequence_privilege('authenticated', format('%I.%I', n.nspname, c.relname), 'usage') as authenticated_usage,",
        "  has_sequence_privilege('service_role', format('%I.%I', n.nspname, c.relname), 'usage') as service_role_usage",
        'from pg_class c',
        'join pg_namespace n on n.oid = c.relnamespace',
        "where n.nspname = 'public'",
        "  and c.relkind = 'S'",
...
function extractStaticStoneFinishImageRows(stoneVariantToGroup, validFinishKeys, blockers) {
    const sourcePath = 'data/clean/stone_finish_images.json';
    const sources = readJson(sourcePath);
    const rows = [];
    for (const [stoneVariantKey, finishes] of Object.entries(sources)) {
        const stoneGroupKey = stoneVariantToGroup.get(stoneVariantKey);
        if (!stoneGroupKey) {
            if (Object.keys(finishes).some((key) => key !== 'default')) blockers.push(`Unknown stone variant image map ${stoneVariantKey} in ${sourcePath}`);
            continue;
        }
        for (const [finishKey, asset] of Object.entries(finishes)) {
            if (finishKey === 'default') continue;
            ...
            [asset, ...(asset.secondaryImages || [])].forEach((image, index) => {
                if (!image.path) return;
                rows.push({
                    stone_group_key: stoneGroupKey,
                    stone_variant_key: stoneVariantKey,
                    finish_key: finishKey,
                    media_source_url: productImageSourceUrl(image.path),
                    image_role: index === 0 ? 'primary' : 'secondary',
                    sort_order: index,
                    status: 'draft',
                    alt: image.alt,
                });
