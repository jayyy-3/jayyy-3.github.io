...
// The quick guide ships inside the admin bundle (Help drawer); editing it must rebuild and deploy.
assert.deepEqual([classify(['docs/ADMIN_EDITOR_GUIDE.md']).category, classify(['docs/ADMIN_EDITOR_GUIDE.md']).deploy], ['runtime', true])
assert.equal(classify(['docs/ADMIN_OPERATIONS_RUNBOOK.md']).deploy, false)
for (const suite of ['docs', 'admin', 'container', 'runtime']) assert.ok(resolveChecks(suite).includes('guide'), `${suite} runs the quick guide check`)
