...
- Production Cloudflare compute is now in the Urblo-owned account (`cb91e0806b808258d6d9c852c8d0f060`) as Pages project `urblo-site`, default domain `urblo-site.pages.dev`. Current main `004445b3b654ec6db0dbcd78a0a7c65b83665d09` is deployed at `https://42cabb15.urblo-site.pages.dev`; both custom domains are attached, SSL-active, and pass exact immutable-reference route/asset/MIME/redirect/Function smoke. The `.au` registry delegates to Urblo-owned nameservers `mckenzie.ns.cloudflare.com` and `norman.ns.cloudflare.com`; the new zone is Active with all 24 required website, Google Workspace, SMTP2GO, SendGrid, Squarespace, and `qa` records. Google Public DNS sees the new pair, while some recursive caches may briefly retain the old delegation. The old Hunter-account project and zone were not deleted and are rollback evidence only. Rollback boundaries are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
