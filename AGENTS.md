...
- Public Projects, Products, Articles, and the Stone Library listing now prefer published Supabase content with static fallback; imported production content is intentionally draft/review-only until an editor publishes rows in `/admin`. Stone Library detail remains static-backed until the deeper public variant/finish detail adapter is completed.
