...
- `npm run agent:forms-live -- --allow-writes` is the credential-gated and approval-gated live form verification runner; it proves valid enquiry/sample-request rows plus audit rows, invalid-payload no-write behavior, and response-vs-stored notification status once a service-role key is available and Jay has approved tagged live form QA writes. With `--require-browser-boundary`, it also requires a browser-safe key and verifies created private lead rows are not anonymously readable. It intentionally fails without `--allow-writes` or required credentials.
...
- `npm run agent:live-readiness` is the no-secret live-input audit runner; it reports which form/admin/Cloudflare live verification inputs are present or missing without printing secrets or mutating Supabase/Cloudflare. Non-secret manual inputs can be passed with `--base-url`, `--admin-email`, `--form-writes-approved`, `--first-admin-writes-approved`, and `--admin-writes-approved`; use `--strict` when missing/manual-gated live inputs should fail the command.
...
- `npm run agent:public-supabase-readiness` is implemented as a no-write public cutover verifier for draft-only import payloads, published-only public RLS policy source, read-only anonymous grants, static public runtime boundaries, and Cloudflare `/api/*` Function routing scope.
