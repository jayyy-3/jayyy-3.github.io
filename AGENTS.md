...
- Local incident repairs address the Supabase auth callback deadlock pattern and same-user background refresh, isolated-session implicit invite/recovery password setup that does not replace the shared browser login, Media record loading/new-record stability, private-only initial uploads, owner/admin create-only private-to-public Storage promotion bound to the selected row's original path/version with reference-safe cleanup disclosure, public Storage URLs, migration-safe per-record overlays, Published settings consumption, parent-bound child updates, loading-state record/bundle locks, the Articles invalid-save lockup, route chunks, medium-desktop header clipping, and Projects record/task-workspace/all-editor dirty/searchable-media UX. These changes require deployment and approved production-prerequisite/golden-workflow verification before handoff.
- Supabase Auth custom SMTP and exact invite/recovery Redirect URL allowlist entries are unverified. Contact/Sample Request SMTP2GO proof is a separate mail path and does not prove admin Auth email delivery.
...
- Cloudflare account access has been rechecked for the production zone: `urblo.com.au` is readable in Hunter's account (`077afae2c6f4e77badadf21e49e58eb7`), the `urblo` Pages project exists with default domain `urblo.pages.dev`, GitHub source is connected to `jayyy-3/jayyy-3.github.io`, and the latest verified CMS code deployment `bc830b5f-4c98-46ab-8962-586478ff9259` for commit `7a318ab` is successful. Production custom domains `urblo.com.au` and `www.urblo.com.au` are attached, `npm run agent:cloudflare-preview-smoke -- --base-url https://urblo.com.au` passes for the current deployment, and website DNS points apex and `www` to `urblo.pages.dev`; Google MX/SPF/TXT, NS records, and `qa.urblo.com.au` were not changed. Rollback DNS values are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
...
- Latest local runtime gates were measured on 2026-07-13 and were green (`npm run build`, `npm run lint`, `npx tsc -b`, `npm run agent:smoke`) for the reopened admin reliability/UX source patch. This is not deployed golden-workflow proof.
