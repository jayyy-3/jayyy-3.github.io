...
- Projects aggregate expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back on production. The separately approved owner workflow on preview deployment `1a3e0d4b-d74a-4979-be64-921e5a510ccc` passed one aggregate Save/refresh, unsaved shared preview, private-first inline image upload, 55/55 visual hotspot, Publish/public readback, and Hide/public-not-found for marker `admin-projects-ui-mrroa6p0`; the tagged Project/draft/children are Archived, the private source object was removed after promotion, and the promoted Media/public object remain by the archive-first retention contract. Project editing remains frozen through tombstone minimum-disclosure resolution or acceptance, aggregate runtime promotion, fresh approval/application/readback of unapplied contract migration `20260719015650_project_aggregate_write_lockdown.sql`, and Jay's fool test.
...
- Latest local runtime gates were measured on 2026-07-13 and were green (`npm run build`, `npm run lint`, `npx tsc -b`, `npm run agent:smoke`) for the reopened admin reliability/UX and cached-asset repair source. PR `#6` merge `a2a7ae5` is deployed as `c7a910df-6dd3-440b-8971-a6120353ed19`; its immutable URL and both custom domains pass the bound MIME/body-aware smoke, and production auth passes three blocked-Supabase static fallbacks plus all nine authenticated routes. The four residual apex cache-header warnings do not mask a byte/MIME mismatch. This is still not authenticated golden-workflow proof.
