...
- `/admin/audit` source is implemented as the first audit visibility screen: active owner/admin roles can inspect audit rows after live auth is configured, but shared mutation helpers still need to write audit events.
...
- Last runtime gates were measured on 2026-05-28 and were green (`npm run build`, `npm run lint`, `npx tsc -b`, `npm run agent:smoke`) during the admin Audit checkpoint.
