...
- `npm run agent:forms-live -- --allow-writes` is the credential-gated and approval-gated live form verification runner; it proves valid enquiry/sample-request rows plus source-route audit metadata, invalid-payload no-row/no-audit behavior, and response-vs-stored notification status. The 2026-06-02 deployed proof used approved tagged writes plus Supabase connector readback for the base persistence/audit case; `--require-browser-boundary`, `--allow-email --require-email`, and `--require-turnstile --turnstile-token <token>` remain separate final proofs once browser-safe Supabase, SMTP2GO/email, and Turnstile inputs are configured.
...
- Cloudflare account access has been rechecked for the production zone: `urblo.com.au` is readable in Hunter's account (`077afae2c6f4e77badadf21e49e58eb7`), the `urblo` Pages project exists with default domain `urblo.pages.dev`, GitHub source is connected to `jayyy-3/jayyy-3.github.io`, and the production redeploy after form env configuration (`17588cfa-2204-4b95-b6e0-4e3531e366bb`) is successful. Latest verified production build for commit `f21bbd4` serves `/assets/index-DZ_ipi64.js`, and production custom domains `urblo.com.au` and `www.urblo.com.au` are attached with both custom-domain smoke checks passing. Website DNS now points apex and `www` to `urblo.pages.dev`; Google MX/SPF/TXT, NS records, and `qa.urblo.com.au` were not changed. Rollback DNS values are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
...
- Follow-up email notification requirement: Contact and Sample Request notifications should go to `info@urblo.com.au`; SMTP2GO is the selected provider path, the Function code also keeps Resend compatibility, and final proof still requires Cloudflare email variables plus approved tagged live form QA emails.
...
- Contact route is declared at `/contact`; shared header/footer navigation points to declared routes, and Sample Request routes to `/contact?intent=sample-request`. The main Contact/Sample Request submit source now posts to Pages Functions; direct email/phone links remain manual contact channels, and basic production persistence/audit behavior is verified. Final email, Turnstile, browser-key privacy boundary, and admin-visible workflow proof remain pending.
