...
- `npm run agent:live-readiness` is the no-secret live-input audit runner; it reports which form/admin/Cloudflare live verification inputs are present or missing without printing secrets or mutating Supabase/Cloudflare. Use `--strict` when missing/manual-gated live inputs should fail the command.
...
- `npm run agent:admin-crud-live` is implemented as a credential-gated live admin write verifier. Default mode is plan-only and no-write; `--allow-writes` requires a real owner/admin Supabase Auth session and creates tagged draft/archived QA rows plus audit events through browser-key RLS.
- `npm run agent:content-import` is implemented as a no-write content import dry run. It maps current static Stone Library, Products, Projects, Articles, and media candidates into Supabase-shaped draft payloads and fails on missing local assets or unknown stone/finish references before any production data import is attempted. Use `npm run agent:content-import -- --out .tmp/content-import-preview.json` for a local ignored review artifact, `npm run agent:content-import:plan` to also write a local ignored Markdown apply/rollback plan, or `npm run agent:content-import:preflight-sql` to also write a local ignored read-only Supabase target preflight SQL artifact.
