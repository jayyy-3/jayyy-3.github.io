...
- `npm run agent:forms-live` is the credential-gated and approval-gated live form verification runner; it proves valid enquiry/sample-request rows plus audit rows, invalid-payload no-write behavior, and response-vs-stored notification status once a service-role key is available and Jay has approved tagged live form QA writes. With `--require-browser-boundary`, it also requires a browser-safe key and verifies created private lead rows are not anonymously readable. It intentionally fails without required credentials.
