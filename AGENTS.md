...
- Cloudflare account access has been rechecked for the production zone: `urblo.com.au` is readable in Hunter's account (`077afae2c6f4e77badadf21e49e58eb7`), the `urblo` Pages project exists with default domain `urblo.pages.dev`, GitHub source is connected to `jayyy-3/jayyy-3.github.io`, and production custom domains `urblo.com.au` and `www.urblo.com.au` are attached with website DNS pointing to `urblo.pages.dev`. The previous custom-domain smoke conclusion is invalidated by the cached-HTML asset incident; use the MIME-aware smoke with the exact immutable deployment passed as `--reference-url` before restoring a green production claim. Google MX/SPF/TXT, NS records, and `qa.urblo.com.au` were not changed. Rollback DNS values are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
...
- Latest local runtime gates were measured on 2026-07-13 and were green (`npm run build`, `npm run lint`, `npx tsc -b`, `npm run agent:smoke`) for the reopened admin reliability/UX and cached-asset repair source. PR `#5` branch preview and immutable deployment pass, but its former production custom-domain smoke is invalidated by the cached-HTML incident. The repaired custom origin must pass MIME-aware smoke bound to its exact immutable deployment plus the configured auth-browser run before production is called green. This is still not authenticated golden-workflow proof.
