...
18. Production Media role migration `20260714050750_media_public_bucket_role_hardening.sql` is applied/read back and its separately approved tagged role proof passed on 2026-07-14. Do not rerun live writes without fresh approval; if the policy changes and Jay approves a new tagged proof, run:
...
- Incident repairs for the Supabase auth callback deadlock pattern and same-user background refresh, isolated-session implicit invite/recovery password setup, Media reliability/private-first promotion, public Storage URLs, per-record overlays, Published settings consumption, parent-bound child updates, loading-state locks, the Articles invalid-save lockup, route chunks, medium-desktop header clipping, and Projects task-workspace UX were deployed through PR `#3` on 2026-07-13. Its then-current status-only production smoke passed for merge commit `46d46b4`, but that result was later invalidated by the cached-HTML asset incident; approved production-prerequisite and authenticated golden-workflow verification are still required before handoff.
- Harness hardening was deployed through PR `#5` / merge `cb0ec9a` / immutable deployment `4aef2ba1-3e00-4e43-b5d6-1ac962fbf02d`. The MIME/body-aware cache repair and immutable-reference binding were then deployed through PR `#6` / merge `a2a7ae5` / deployment `c7a910df-6dd3-440b-8971-a6120353ed19`. The immutable deployment plus `urblo.com.au` and `www.urblo.com.au` pass the bound asset smoke. The apex readback still reports four unchanged assets with the removed long-lived response header, but each warning is allowed only after its bytes and MIME exactly match the immutable deployment; the latest `www` readback was warning-free. This restores no-write runtime integrity, not the CMS golden-workflow handoff.
- A real Supabase Auth invite reached the approved QA recipient on 2026-07-13, but its callback fell back to `http://localhost:3000` even though the invite Function supplied the production origin. On 2026-07-14, after Jay's item-specific approval, production Auth Site URL was changed from `http://localhost:3000` to `https://urblo.com.au` and the exact invite/recovery account-setup Redirect URLs were added and read back. Custom SMTP ownership and a repeated invite/recovery golden workflow remain unverified. Contact/Sample Request SMTP2GO proof is a separate mail path and does not prove production-ready admin Auth delivery.
...
- Supabase admin settings/profile/helper hardening migrations are applied and verified: `site_settings` writes are owner/admin only, admin profile owner-role changes are owner-protected, admin profile emails are case-insensitively unique, public helper RPC execution is revoked from browser roles, and RLS/Storage policies call private SECURITY DEFINER helpers. The 2026-07-14 security advisor readback reports one separate Auth warning because leaked-password protection is disabled; no migration-specific Storage/RLS security lint was introduced.
- Supabase media Storage buckets/listing/role hardening are applied and verified: `urblo-public-media` and `urblo-admin-media` exist, public object listing is disabled, and production migration `supabase/migrations/20260714050750_media_public_bucket_role_hardening.sql` retains Editor insert/update in the private draft bucket while restricting public-bucket insert/update to owner/admin. Migration history and both policy definitions are read back; the separately approved live Editor/owner proof passed on 2026-07-14 and independent SQL found zero tagged objects remaining.
- Projects aggregate expand migration `supabase/migrations/20260719015649_project_aggregate_drafts.sql` is applied/read back on production. The separately approved owner workflow on preview deployment `1a3e0d4b-d74a-4979-be64-921e5a510ccc` passed one aggregate Save/refresh, unsaved shared preview, private-first inline image upload, 55/55 visual hotspot, Publish/public readback, and Hide/public-not-found for marker `admin-projects-ui-mrroa6p0`; because that marker is not one of the five bundled Project slugs, it did not prove live static-fallback tombstone suppression. The tagged Project/draft/children are Archived, the private source object was removed after promotion, and the promoted Media/public object remain by the archive-first retention contract. Source migration `supabase/migrations/20260719015650_restrict_archived_project_tombstones.sql` now limits the public RPC to archived canonical rows intersecting the five already-public fallback slugs, but it is not applied and needs fresh approval/readback. Project editing remains frozen through that repair, aggregate runtime promotion, fresh approval/application/readback of unapplied contract migration `supabase/migrations/20260719015651_project_aggregate_write_lockdown.sql`, and Jay's fool test.
- `npm run agent:admin-media-role-boundary-live` is the dedicated browser-key/RLS proof for that policy. It is no-network/no-write by default; live mode requires fresh explicit approval, distinct active Editor and owner/admin credentials, proves Editor private insert/update success plus public insert/update denial, proves owner/admin public insert/update success, and fails unless every tagged Storage object is removed. Origin readback uses a unique cache nonce so Supabase CDN invalidation delay cannot masquerade as a Storage object or stale update.
...
- `npm run agent:admin-auth-browser -- --allow-login --strict --base-url https://urblo.com.au` passed again on 2026-07-13 against the PR `#6` production runtime: it proved static fallback for Products, Projects, and Articles while the Supabase chunk was blocked, signed in through production `/admin/login`, verified all 9 authenticated admin route shells, verified Sign out plus protected-route revisit, and created no content rows/Storage objects/audit events.
...
- Cloudflare account access has been rechecked for the production zone: `urblo.com.au` is readable in Hunter's account (`077afae2c6f4e77badadf21e49e58eb7`), the `urblo` Pages project exists with default domain `urblo.pages.dev`, GitHub source is connected to `jayyy-3/jayyy-3.github.io`, and production custom domains `urblo.com.au` and `www.urblo.com.au` are attached with website DNS pointing to `urblo.pages.dev`. PR `#6` deployment `c7a910df-6dd3-440b-8971-a6120353ed19` is the latest verified runtime; both custom domains pass the MIME/body-aware smoke bound to its immutable URL. Four apex assets still expose stale long-lived cache headers but match the immutable bytes/MIME exactly, so they remain a visible cleanup warning rather than a content-integrity failure. Google MX/SPF/TXT, NS records, and `qa.urblo.com.au` were not changed. Rollback DNS values are recorded in `docs/CLOUDFLARE_DEPLOYMENT.md`.
...
- `/admin/settings` source can read/create/update the default `site_settings` row and manage existing Supabase Auth users' admin profile rows for owner/admin roles. First admin bootstrap, production browser configuration, and active-admin browser QA are complete for `info@urblo.com.au`; fresh Settings/team-management UI golden-workflow evidence remains open.
...
- `/admin/stone-library` source is implemented as the first content CRUD screen: active admin/editor roles can maintain stone groups, variants, finish capability rows, and finish image links to media records. Fresh deployed UI golden-workflow evidence remains open.
- `/admin/projects` now uses the page-shaped aggregate editor and protected Function/RPC boundary. Its authenticated Preview happy path passed, but the aggregate runtime is not promoted to production, C/B remain unapplied, and Project editing stays frozen.
- `/admin/products` source is implemented as the next content CRUD screen: active admin/editor roles can maintain product families, models, material defaults, and specs. Fresh deployed UI golden-workflow evidence remains open.
- `/admin/articles` source is implemented as the next content CRUD screen: active admin/editor roles can maintain article metadata and structured article blocks. Fresh deployed UI golden-workflow evidence remains open.
- `/admin/leads` source is implemented as the first lead inbox screen: active owner/admin roles can update enquiry and sample request status, assignment, internal notes, and export the currently loaded queue to CSV. CSV export is blocked unless an audit event is recorded first; fresh deployed UI golden-workflow evidence remains open.
- `/admin/audit` source is implemented as the first audit visibility screen: active owner/admin roles can inspect audit rows, and admin CRUD/workflow save flows call a shared audit writer after successful primary mutations. Fresh deployed UI golden-workflow evidence remains open.
...
- Latest host-side runtime gates were measured on 2026-07-19 and are green (`npm run build`, `npm run lint`, `npx tsc -b`, `npm run agent:smoke`, Admin predeploy/config gates) for the current unpushed Projects closeout source. Separately, PR `#6` merge `a2a7ae5` was deployed on 2026-07-13 as `c7a910df-6dd3-440b-8971-a6120353ed19`; its immutable URL and both custom domains pass the bound MIME/body-aware smoke, and production auth passes three blocked-Supabase static fallbacks plus all nine authenticated routes. The four residual apex cache-header warnings do not mask a byte/MIME mismatch. Neither evidence set is authenticated production golden-workflow proof.
