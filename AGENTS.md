...
- `npm run agent:content-import` is implemented as a no-write content import dry run. It maps current static Stone Library, Products, Projects, Articles, and media candidates into Supabase-shaped payloads and fails on missing local assets or unknown stone/finish references before any production data import is attempted.
...
- Last runtime gates were measured on 2026-05-28 and were green (`npm run build`, `npm run lint`, `npx tsc -b`, `npm run agent:smoke`) during the server-side form audit checkpoint.
