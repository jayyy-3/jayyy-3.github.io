...
- `/admin/settings` source can read/create/update the default `site_settings` row and manage existing Supabase Auth users' admin profile rows for owner/admin roles after live auth is configured; first admin bootstrap is complete, and active-admin browser QA now requires a real `info@urblo.com.au` login session/password supplied outside chat.
